# NOVA AI PR Review Instructions

You are reviewing pull requests for NOVA AI, a local-first personal AI agent framework built with Python, Rust (PyO3), and TypeScript.
...
- **Registry pattern compliance** — new components (engines, tools, agents, channels) must register via `ToolRegistry`, `EngineRegistry`, `AgentRegistry`, `ChannelRegistry`, etc. in `src/nova_ai/core/registry.py`
...
- **Event bus integration** — new lifecycle events should use `EventBus` from `src/nova_ai/core/events.py`
