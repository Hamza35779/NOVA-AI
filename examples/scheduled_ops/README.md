...
This tutorial demonstrates how to use NOVA AI to run **autonomous scheduled agents** that perform recurring personal tasks on a cron-like schedule.

## What This Demonstrates

- Using the `Nova` SDK with different agent types (`orchestrator`, `native_react`) and tool sets
- Configuring recurring schedules via TOML and the `nova scheduler` CLI
...
Register each script as a recurring task with `nova scheduler create`:
...
uv run python examples/scheduled_ops/daily_digest.py | nova channel send slack
...
See `nova channel list` for all available channels.
...
- **Model selection**: Use `--model` to target a specific model, or let NOVA AI auto-select from what is available.
