...
using NOVA AI. The assistant uses an orchestrator agent loop with web
...
- NOVA AI installed (`uv sync --extra dev` from the repo root)
...
recipe that can be loaded with `load_recipe()` or passed to the `nova eval`
runner.
...
User query
  |
  v
Nova SDK  (model + engine selection)
  |
  v
...
- **Use the recipe programmatically** -- load the TOML with
  `nova_ai.recipes.load_recipe("examples/deep_research/research.toml")` and
