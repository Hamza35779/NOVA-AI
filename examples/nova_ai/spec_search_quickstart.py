...
are local fakes, so you can ``python examples/nova_ai/spec_search_quickstart.py``
with no API keys and no Ollama / vLLM running. Every "swap this for production"
hookpoint is called out inline.

Configuration is read from ``configs/nova_ai/examples/spec-search-quickstart.toml``
via the regular ``nova_ai.core.config.load_config`` machinery — you can copy
that TOML to ``~/.nova_ai/config.toml`` and tune the gate / stagnation /
reward knobs without editing this file.

Run:

    NOVA_AI_HOME=/tmp/nova_ai-spec-search-demo \\
...
from nova_ai.core.config import SpecSearchLearningConfig
from nova_ai.learning.spec_search.composite_reward import (
    RewardWeights,
    TrainingSample,
    score_batch,
)
...
from nova_ai.learning.spec_search.multi_session import SpecSearchLoop
from nova_ai.learning.spec_search.orchestrator import SpecSearchOrchestrator
from nova_ai.learning.spec_search.triggers import OnDemandTrigger
...
def _fake_diagnosis() -> Any:
    ...
    from nova_ai.learning.spec_search.diagnose.runner import DiagnosisResult
...
def build_orchestrator(
    ...
    return SpecSearchOrchestrator.from_config(
        ...
        # Judge: production = nova_ai.evals.core.scorer.LLMJudgeScorer(...)
        judge=MagicMock(),
...
def main() -> None:
    # In a real deployment, ``load_config()`` reads from ``~/.nova_ai/config.toml``.
    ...
    #
    # (after copying ``configs/nova_ai/examples/spec-search-quickstart.toml``
    # to ``~/.nova_ai/config.toml``)
    ...
    home = Path(
        os.environ.get("NOVA_AI_HOME")
        or tempfile.mkdtemp(prefix="nova_ai-spec-search-")
    )
    print(f"NOVA_AI_HOME = {home}")
    ...
    with patch(
        "nova_ai.learning.spec_search.orchestrator.DiagnosisRunner"
    ) as MockDiag:
