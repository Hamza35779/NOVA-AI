...
def _run_channel(channel: str, model: str, engine_key: str) -> None:
    ...
    help_text = setup_help.get(
        ...
        f"Run: nova channel list   to see available channels.\n",
    )
    ...
    click.echo("  from nova_ai import Nova")
    click.echo(f'  j = Nova(model="{model}", engine_key="{engine_key}")')
    click.echo("  # Listen for incoming messages on the channel")
    click.echo(f'  # See: nova channel status  (to verify "{channel}" is connected)')
