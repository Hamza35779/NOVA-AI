...
from nova_ai.core.types import Message, Role, TelemetryRecord
from nova_ai.telemetry.aggregator import AggregatedStats, TelemetryAggregator
...
_bench_mod = importlib.import_module("nova_ai.cli.bench_cmd")
...
class TestCliAskWiring:
    ...
    def test_energy_monitor_created_when_gpu_metrics_on(
        ...
        with patch(
            "nova_ai.telemetry.energy_monitor.create_energy_monitor",
...
class TestBenchWiring:
    ...
    def test_energy_monitor_passed_to_benchmarks(self):
        ...
        with (
            patch(
                "nova_ai.cli.bench_cmd.get_engine",
                return_value=("mock", engine),
            ),
            patch(
                "nova_ai.cli.bench_cmd.load_config",
                return_value=cfg,
            ),
            patch(
                "nova_ai.telemetry.energy_monitor.create_energy_monitor",
...
def _patch_telemetry_config(tmp_path: Path):
    ...
    return mock.patch(
        "nova_ai.cli.telemetry_cmd.load_config",
        return_value=cfg,
    ), db_path
