...
class TestRateLimit:
    def test_skips_execution_within_min_interval(self, store):
        from datetime import timedelta

        mock_system = MagicMock()
        mock_system.ask.return_value = "ok"
        sched = TaskScheduler(store, system=mock_system, poll_interval=1)

        task = sched.create_task("tick", "interval", "5")
        d = store.get_task(task.id)
        d["metadata"]["rate_limit_rpm"] = 6  # min interval: 10s
        d["last_run"] = (datetime.now(timezone.utc) - timedelta(seconds=3)).isoformat()
        store.update_task(d)

        sched._execute_task(ScheduledTask.from_dict(store.get_task(task.id)))

        mock_system.ask.assert_not_called()
        # next_run pushed past the minimum interval instead of spinning
        updated = store.get_task(task.id)
        assert updated["last_run"] == d["last_run"]
        assert updated["next_run"] > d["last_run"]

    def test_runs_once_min_interval_elapsed(self, store):
        from datetime import timedelta
        ...
        task = sched.create_task("tick", "interval", "60")
        d = store.get_task(task.id)
        d["metadata"]["rate_limit_rpm"] = 60  # min interval: 1s
        d["last_run"] = (datetime.now(timezone.utc) - timedelta(seconds=30)).isoformat()
        store.update_task(d)
    ...
    def test_no_limit_when_unset(self, store):
        mock_system = MagicMock()
        mock_system.ask.return_value = "ok"
        sched = TaskScheduler(store, system=mock_system, poll_interval=1)
    ...
    def test_invalid_rpm_ignored(self, store):
        from datetime import timedelta
        ...
        task = sched.create_task("tick", "interval", "60")
        d = store.get_task(task.id)
        d["metadata"]["rate_limit_rpm"] = "not-a-number"
        d["last_run"] = (datetime.now(timezone.utc) - timedelta(seconds=1)).isoformat()
        store.update_task(d)
...
# -- Circuit breaker (consecutive failure auto-pause) --------------------------


class TestCircuitBreaker:
    def _failing_task(self, sched, store, max_failures=3):
        task = sched.create_task("boom-work", "interval", "60")
        d = store.get_task(task.id)
        d["metadata"]["max_consecutive_failures"] = max_failures
        store.update_task(d)
        return ScheduledTask.from_dict(store.get_task(task.id))

    def test_pauses_after_consecutive_failures(self, store):
        mock_system = MagicMock()
        mock_system.ask.side_effect = RuntimeError("engine down")
        sched = TaskScheduler(store, system=mock_system, poll_interval=1)
        task = self._failing_task(sched, store, max_failures=3)

        for _ in range(3):
            fresh = ScheduledTask.from_dict(store.get_task(task.id))
            fresh.last_run = None  # keep it due
            sched._execute_task(fresh)

        assert store.get_task(task.id)["status"] == "paused"

    def test_not_paused_below_threshold(self, store):
        mock_system = MagicMock()
        mock_system.ask.side_effect = RuntimeError("engine down")
        sched = TaskScheduler(store, system=mock_system, poll_interval=1)
        task = self._failing_task(sched, store, max_failures=3)
    ...
    def test_success_resets_failure_streak(self, store):
        mock_system = MagicMock()
        responses = iter([RuntimeError("x"), "recovered"])
        mock_system.ask.side_effect = lambda *a, **kw: raise_or_return(next(responses))
        ...
        sched = TaskScheduler(store, system=mock_system, poll_interval=1)
        task = self._failing_task(sched, store, max_failures=2)

        fresh = ScheduledTask.from_dict(store.get_task(task.id))
        fresh.last_run = None
        sched._execute_task(fresh)  # failure 1
        fresh = ScheduledTask.from_dict(store.get_task(task.id))
        fresh.last_run = None
        sched._execute_task(fresh)  # success resets streak

        fresh = ScheduledTask.from_dict(store.get_task(task.id))
        fresh.last_run = None
        sched._execute_task(fresh)  # failure — streak is only 1

        assert store.get_task(task.id)["status"] == "active"

    def test_publishes_paused_event(self, store):
        from nova_ai.core.events import EventBus
        from nova_ai.scheduler.scheduler import SCHEDULER_TASK_PAUSED

        bus = EventBus()
        seen = []
        bus.subscribe(SCHEDULER_TASK_PAUSED, lambda e: seen.append(e.data))

        mock_system = MagicMock()
        mock_system.ask.side_effect = RuntimeError("down")
        sched = TaskScheduler(store, system=mock_system, poll_interval=1, bus=bus)
        task = self._failing_task(sched, store, max_failures=1)
        ...
        assert seen[0]["reason"] == "consecutive_failures"
        assert "down" in seen[0]["error"]

    def test_disabled_by_default(self, store):
        mock_system = MagicMock()
        mock_system.ask.side_effect = RuntimeError("down")
        sched = TaskScheduler(store, system=mock_system, poll_interval=1)
        task = sched.create_task("no-breaker", "interval", "60")
...
class TestRunLogPassthrough:
    def test_get_run_logs_delegates_to_store(self, scheduler, store):
        task = scheduler.create_task("t", "once", "2026-01-01T00:00:00+00:00")
        scheduler._execute_task(ScheduledTask.from_dict(store.get_task(task.id)))
        logs = scheduler.get_run_logs(task.id)
        assert len(logs) == 1
