...
``~/.nova_ai/connectors`` (a real source of spurious failures — see the
...
def hermetic_connectors(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    *source* (``nova_ai.core.config``) — every connector re-derives
    ...
    import nova_ai.connectors.oauth as oauth_mod
    import nova_ai.core.config as config_mod
    import nova_ai.server.connectors_router as router_mod
    ...
    google_mods = [
        "nova_ai.connectors.gdrive",
        "nova_ai.connectors.gcalendar",
        "nova_ai.connectors.gcontacts",
        "nova_ai.connectors.gmail",
        "nova_ai.connectors.google_tasks",
    ]
    ...
    # subsequent tests in the same process see ~/.nova_ai again.
    for name in google_mods:
        if name in sys.modules:
            importlib.reload(sys.modules[name])
...
def test_connect_client_pair_returns_oauth_required_no_browser(
    ...
    """
    with patch("nova_ai.core.open_browser") as mock_browser:
        resp = client.post(
...
def test_oauth_callback_exchanges_and_connects(
    ...
    from nova_ai.connectors.gdrive import GDriveConnector
