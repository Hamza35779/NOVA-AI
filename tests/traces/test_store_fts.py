...
def store():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        # Close before TemporaryDirectory cleanup: on Windows an open SQLite
        # handle keeps traces.db locked and the cleanup raises PermissionError.
        with TraceStore(Path(tmpdir) / "traces.db") as s:
            yield s
