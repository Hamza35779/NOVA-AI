...
class TestWebSearchTool:
    ...
    def test_execute_no_api_key(self, monkeypatch):
        ...
        Both engines are stubbed: a real Tavily client would need network,
        and a real DDG call would clear-environment-crash on Windows (the
        ddgs package's Rust DNS resolver requires SystemRoot).
        """
        mock_tavily_module = MagicMock()
        mock_tavily_module.TavilyClient.side_effect = RuntimeError("no api key")
        monkeypatch.setitem(sys.modules, "tavily", mock_tavily_module)

        tool = WebSearchTool(api_key=None)
        with patch.object(
            WebSearchTool, "_duckduckgo_search", return_value="ddg results"
        ):
...
class TestUrlFetching:
    def _mock_ssrf(self, monkeypatch):
        ...
        import nova_ai.tools.web_search as _ws
