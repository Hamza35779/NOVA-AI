...
from nova_ai.tools.templates.loader import (
    ToolTemplate,
    discover_templates,
    safe_eval_expr,
)
...
def test_shell_action_neutralizes_injection(tmp_path) -> None:
    ...
    if os.name == "nt":
        pytest.skip("`echo` is a POSIX builtin; Windows ships no echo.exe and "
                    "the loader intentionally runs argv without a shell")
