"""Verify that importing nova_ai.tools registers all built-in tools."""
...
def _reload_tool_modules() -> None:
    """Reload all nova_ai.tools.* submodules to re-trigger @register decorators.

    The autouse ``_clean_registries`` fixture clears all registries before each
    test.  A plain ``import nova_ai.tools`` won't re-register because the
