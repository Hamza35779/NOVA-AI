...
setup() {
    ...
    export NOVA_AI_HOME="$FAKE_HOME/.nova_ai"
    ...
    # uv stub needs to fake creating a venv with a nova binary.
...
case "\$1" in
    venv)
        ...
        if [[ -n "\$venv_path" ]]; then
            ...
            cat > "\$venv_path/bin/nova" <<'EOJ'
#!/usr/bin/env bash
# fake nova for tests
echo "fake nova: \$@"
exit 0
EOJ
            chmod +x "\$venv_path/bin/nova"
...
@test "creates ~/.nova_ai directory tree" {
    run bash "$SCRIPT" --no-bg-orchestrator
    [ "$status" -eq 0 ]
    [ -d "$NOVA_AI_HOME/src" ]
    [ -d "$NOVA_AI_HOME/.state" ]
    [ -d "$NOVA_AI_HOME/.scripts" ]
}

@test "writes install-state.json on success" {
    run bash "$SCRIPT" --no-bg-orchestrator
    [ "$status" -eq 0 ]
    [ -f "$NOVA_AI_HOME/.state/install-state.json" ]
}

@test "calls git clone for the NOVA AI repo" {
    run bash "$SCRIPT" --no-bg-orchestrator
    grep -q "clone" "$GIT_STUB_LOG"
}

@test "creates nova symlink in ~/.local/bin" {
    run bash "$SCRIPT" --no-bg-orchestrator
    [ "$status" -eq 0 ]
    [ -L "$FAKE_HOME/.local/bin/nova" ] || [ -f "$FAKE_HOME/.local/bin/nova" ]
}
...
@test "detects WSL2 and writes platform note to install-state" {
    NOVA_AI_FORCE_WSL=1 run bash "$SCRIPT" --no-bg-orchestrator
    [ "$status" -eq 0 ]
    grep -q "wsl" "$NOVA_AI_HOME/.state/install-state.json"
}
...
@test "install succeeds with no system Python on PATH (#484)" {
    ...
    grep -q '"install_uv":' "$NOVA_AI_HOME/.state/install-state.json"
}
...
@test "mark_done is idempotent — second mark of same key doesn't duplicate" {
    ...
    local first_count
    first_count="$(grep -c '"install_uv":' "$NOVA_AI_HOME/.state/install-state.json")"
    [ "$first_count" -eq 1 ]
    # Second run reuses state — keys should not duplicate even if some path re-marks.
    run bash "$SCRIPT" --no-bg-orchestrator
    [ "$status" -eq 0 ]
    local second_count
    second_count="$(grep -c '"install_uv":' "$NOVA_AI_HOME/.state/install-state.json")"
    [ "$second_count" -eq 1 ]
}
