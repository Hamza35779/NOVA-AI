...
setup() {
    ...
    export NOVA_AI_HOME="$TEST_TMPDIR/.nova_ai"
    mkdir -p "$NOVA_AI_HOME/.state/models"
    mkdir -p "$NOVA_AI_HOME/.scripts"
    # Provide mock scripts that all return success and write the expected state files.
    cat > "$NOVA_AI_HOME/.scripts/install-rust.sh" <<EOF
#!/usr/bin/env bash
exit 0
EOF
    cat > "$NOVA_AI_HOME/.scripts/build-extension.sh" <<EOF
#!/usr/bin/env bash
touch "\$NOVA_AI_HOME/.state/extension-built"
exit 0
EOF
    cat > "$NOVA_AI_HOME/.scripts/pull-model.sh" <<EOF
#!/usr/bin/env bash
touch "\$NOVA_AI_HOME/.state/models/\${1}.ready"
exit 0
EOF
    chmod +x "$NOVA_AI_HOME/.scripts/"*.sh
...
@test "writes pid file" {
    ...
    [ -f "$NOVA_AI_HOME/.state/bg-orchestrator.log" ]
    grep -q "pid=" "$NOVA_AI_HOME/.state/bg-orchestrator.log"
}

@test "invokes build-extension after install-rust succeeds" {
    run bash "$SCRIPT" qwen3.5:9b
    [ "$status" -eq 0 ]
    [ -f "$NOVA_AI_HOME/.state/extension-built" ]
}
