...
from nova_ai.core import get_python_executable
from nova_ai.security.subprocess_sandbox import (
    build_safe_env,
    kill_process_tree,
    run_sandboxed,
)
...
def _quoted_python() -> str:
    """Quote the interpreter path appropriately for the platform shell."""
    path = get_python_executable()
    return f'"{path}"' if IS_WINDOWS else shlex.quote(path)
...
class TestBuildSafeEnv:
    def test_only_safe_vars_included(self) -> None:
        ...
        safe_keys = {
            ...
            "TMPDIR",
            "TZ",
            # Windows essentials (see _SAFE_ENV_VARS)
            "SYSTEMROOT",
            "SYSTEMDRIVE",
            "COMSPEC",
            "USERPROFILE",
    ...
    def test_secrets_never_pass_through(self) -> None:
        os.environ["CANARY_NOCOPY_TOKEN"] = "leak-me-not"
        try:
            assert "CANARY_NOCOPY_TOKEN" not in build_safe_env()
        finally:
            del os.environ["CANARY_NOCOPY_TOKEN"]
...
class TestRunSandboxed:
    ...
    def test_timeout_kills_process(self) -> None:
        cmd = (
            f"{_quoted_python()} -c \"import time; time.sleep(60)\""
            if IS_WINDOWS
            else "sleep 60"
        )
    ...
    def test_working_dir(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            # `pwd` is POSIX-only; use the interpreter for a portable cwd probe.
            cmd = f'{_quoted_python()} -c "import os; print(os.getcwd())"'
            result = run_sandboxed(cmd, working_dir=tmpdir, timeout=10.0)
            assert result.returncode == 0
            assert tmpdir.lower() in result.stdout.strip().lower()

    def test_env_isolation(self) -> None:
        os.environ["TEST_SECRET"] = "super_secret_value"
        try:
            expansion = "%TEST_SECRET%" if IS_WINDOWS else "$TEST_SECRET"
            result = run_sandboxed(
                f'echo "val={expansion}"',
                timeout=10.0,
            )
            assert result.returncode == 0
    ...
    def test_output_truncation(self) -> None:
        ...
        result = run_sandboxed(
            f'{_quoted_python()} -c "print(\'A\' * 200)"',
            timeout=10.0,
            max_output_bytes=50,
        )
        assert result.returncode == 0
