...
Presets are installed via `nova init --preset <name>`, which copies
`configs/nova_ai/examples/<name>.toml` to `~/.nova_ai/config.toml`.
...
PRESETS_DIR = Path(__file__).resolve().parents[2] / "configs" / "nova_ai" / "examples"
...
def test_preset_loads(preset_path: Path) -> None:
    ...
    # slots `nova init` expects to be populated for a working first run.
