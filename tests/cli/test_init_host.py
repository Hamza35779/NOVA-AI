"""Tests for ``nova init --host`` option."""
...
from nova_ai.core.config import generate_default_toml, generate_minimal_toml
...
class TestInitHost:
    def test_init_host_writes_to_config(self, tmp_path: Path) -> None:
        """nova init --host writes the host into config.toml."""
    ...
    def test_init_host_with_vllm(self, tmp_path: Path) -> None:
        """nova init --host applies to the selected engine."""
        config_dir = tmp_path / ".nova_ai"
        config_path = config_dir / "config.toml"
        with (
            mock.patch("nova_ai.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
    ...
    def test_init_host_probes_and_reports(self, tmp_path: Path) -> None:
        """nova init --host shows reachability status."""
        config_dir = tmp_path / ".nova_ai"
        config_path = config_dir / "config.toml"
        with (
            mock.patch("nova_ai.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
            mock.patch("nova_ai.cli.init_cmd.DEFAULT_CONFIG_PATH", config_path),
            mock.patch("nova_ai.cli.init_cmd.PrivacyScanner"),
            mock.patch("nova_ai.cli.init_cmd.httpx") as mock_httpx,
        ):
    ...
    def test_init_without_host_still_works(self, tmp_path: Path) -> None:
        """nova init without --host still produces valid config."""
        config_dir = tmp_path / ".nova_ai"
        config_path = config_dir / "config.toml"
        with (
            mock.patch("nova_ai.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
