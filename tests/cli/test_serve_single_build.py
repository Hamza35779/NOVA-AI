...
# ``nova_ai.cli.serve`` as an attribute resolves to the click *command*
# (re-exported on the package); grab the real module to monkeypatch its globals.
serve_mod = importlib.import_module("nova_ai.cli.serve")
...
def _repopulate_registries() -> None:
    ...
    from nova_ai.core.registry import (
        AgentRegistry,
        MemoryRegistry,
        ToolRegistry,
    )
    ...
    if not AgentRegistry.keys():
        for mod_name in list(sys.modules):
            if mod_name.startswith("nova_ai.agents.") and not mod_name.endswith(
                "_stubs"
            ):
                try:
    ...
    if not MemoryRegistry.keys():
        for mod_name in list(sys.modules):
            if mod_name.startswith("nova_ai.tools.storage.") and not mod_name.endswith(
                "_stubs"
            ):
                try:
                    importlib.reload(sys.modules[mod_name])
...
def _run_serve(tmp_path, monkeypatch, *, build_spy, set_system_spy):
    ...
    monkeypatch.setattr("nova_ai.security.setup_security", lambda *a, **k: sec)

    with (
        patch(
            "nova_ai.system.builder.SystemBuilder.build",
            build_spy,
        ),
        patch(
            "nova_ai.agents.executor.AgentExecutor.set_system",
            set_system_spy,
        ),
