"""Tests for the ``nova eval`` CLI commands."""
...
class TestEvalCLI:
    ...
    def test_eval_group_exists(self):
        """``nova eval --help`` shows run/compare/report/list subcommands."""
    ...
    def test_eval_list_benchmarks(self):
        """``nova eval list`` exits 0 and outputs benchmark names."""
        ...
        assert "nova-direct" in result.output
        assert "nova-agent" in result.output

    def test_eval_run_missing_args(self):
        """``nova eval run`` without required args fails gracefully."""
