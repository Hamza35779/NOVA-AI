...
from nova_ai.cli import _version_check
from nova_ai.cli._version_check import (
    _check_disabled,
    _config_disabled,
...
def _clean_env(monkeypatch, tmp_path):
    for v in ("NOVA_AI_NO_UPDATE_CHECK", "CI", "NOVA_AI_CONFIG"):
        monkeypatch.delenv(v, raising=False)
    # Point config + cache at empty tmp paths so tests don't see the
    # developer's real ~/.nova_ai state.
    monkeypatch.setenv("NOVA_AI_CONFIG", str(tmp_path / "no-config.toml"))
...
class TestCheckDisabled:
    ...
    def test_nova_no_update_check_disables(self, monkeypatch, value):
        monkeypatch.setenv("NOVA_AI_NO_UPDATE_CHECK", value)
...
class TestConfigDisabled:
    ...
    def test_auto_update_false_disables(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("NOVA_AI_CONFIG", str(cfg))
    ...
    def test_nova_ai_config_env_override(self, monkeypatch, tmp_path):
        """NOVA_AI_CONFIG should redirect the lookup, matching core.config."""
...
class TestCheckForUpdates:
    @patch("nova_ai.cli._version_check._do_check")
    ...
    @patch(
        "nova_ai.cli._version_check._do_check",
