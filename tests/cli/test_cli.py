...
import nova_ai
from nova_ai.cli import cli, main


class TestMainEntryPoint:
    """Tests for the ``nova`` console script entry point."""
    ...
    def test_windows_reconfigures_stdout_to_utf8(self) -> None:
        ...
        with (
            ...
            mock.patch("nova_ai.cli.cli") as cli_mock,
        ):
            main()
...
class TestCLI:
    def test_help(self) -> None:
        ...
        assert "NOVA AI" in result.output
...
class TestStartupResilience:
    ...
    down every `nova` command — including `nova serve` — because the CLI
    ...
    def test_importing_cli_does_not_import_numpy(self) -> None:
        ...
        assert result.returncode == 0, (
            "importing nova_ai.cli pulled in numpy (a broken numpy would then "
