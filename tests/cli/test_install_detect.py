...
from nova_ai.cli._install_detect import InstallInfo, detect_install


def _patch_pkg_file(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Point ``nova_ai.__file__`` at ``tmp_path / nova_ai / __init__.py``."""
    pkg_dir = tmp_path / "nova_ai"
...
def test_uv_tool_install_detected(tmp_path, monkeypatch):
    fake = tmp_path / "share" / "uv" / "tools" / "nova_ai" / "lib" / "python3.12"
...
def test_missing_nova_ai_file_falls_back_to_pypi(monkeypatch):
    """nova_ai unimportable / no __file__ — still get a sane default."""
    with patch("nova_ai.cli._install_detect.Path") as mock_path:
