...
from nova_ai.server.api_routes import include_all_routes  # noqa: E402
...
class TestMemoryRoutes:
    def test_search(self):
        ...
        # 200: backend answered; 503: nova_ai_rust extension missing from this
        # venv (documented degraded mode, see #502 and TestMemoryRustMissing);
        # 500: SQLite not set up.
...
class TestMemoryRustMissing:
    """Regression for #502: when the native ``nova_ai_rust`` extension is
    ...
    def _client(monkeypatch):
        ...
        def _boom():
            raise ImportError("No module named 'nova_ai_rust'")

        import nova_ai._rust_bridge as bridge
    ...
    def test_config_reports_unavailable(self, monkeypatch):
        ...
        assert "nova_ai_rust" in (data["detail"] or "")
...
class TestMetricsRoute:
    def test_metrics_endpoint(self):
        ...
        assert resp.status_code == 200
        assert "nova_ai" in resp.text or "No metrics" in resp.text
