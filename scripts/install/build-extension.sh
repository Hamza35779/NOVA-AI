...
#
# State files under $NOVA_AI_HOME/.state/:
...
MANIFEST="$SRC_DIR/rust/crates/nova_ai-python/Cargo.toml"
...
if uv run maturin develop -m "$MANIFEST" >>"$LOG" 2>&1; then
    ...
    if ! uv run python -c "import nova_ai_rust" >>"$LOG" 2>&1; then
        rc=$?
        {
            echo "build-extension.sh: maturin succeeded but 'import nova_ai_rust'"
