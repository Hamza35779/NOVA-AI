...
#   NOVA_AI_HOME        Install dir (default: $HOME/.nova_ai)
#   NOVA_AI_REPO_URL    git repo URL (default: https://github.com/Hamza35779/NOVA-AI.git)
#   NOVA_AI_FORCE_WSL   Set 1 to force WSL detection (testing)
...
# uv/git tooling installs to Windows paths the rest of NOVA AI can't
...
NOVA AI runs on Windows via WSL2. Two paths:
...
See the WSL2 install guide for the full walkthrough:
  https://hamza35779.github.io/NOVA-AI/getting-started/wsl2/
EOF
        exit 1
        ;;
esac
...
NOVA AI installs to $HOME/.nova_ai, not /usr/local. Re-run as your
...
# ---- env ----
# NOVA AI keeps ALL of its state (install tree + runtime data, configs,
# databases, caches, logs) under a single root so it never clutters $HOME
# beyond one directory. Relocate it by exporting NOVA_AI_HOME before
# running the installer, e.g.:
#     NOVA_AI_HOME=~/apps/nova_ai curl ... | bash
# The Python runtime honors the same override (and, when NOVA_AI_HOME is
# unset, $XDG_DATA_HOME/nova_ai if XDG_DATA_HOME is set). With nothing set
# the root is ~/.nova_ai, so existing installs are untouched.
NOVA_AI_HOME="${NOVA_AI_HOME:-$HOME/.nova_ai}"
NOVA_AI_REPO_URL="${NOVA_AI_REPO_URL:-https://github.com/Hamza35779/NOVA-AI.git}"
SRC_DIR="$NOVA_AI_HOME/src"
VENV_DIR="$NOVA_AI_HOME/.venv"
STATE_DIR="$NOVA_AI_HOME/.state"
SCRIPTS_DIR="$NOVA_AI_HOME/.scripts"
STATE_FILE="$STATE_DIR/install-state.json"

mkdir -p "$NOVA_AI_HOME" "$STATE_DIR" "$SCRIPTS_DIR"

# ---- WSL detection ----
WSL=0
if [[ "${NOVA_AI_FORCE_WSL:-0}" == "1" ]]; then
    WSL=1
...
# NOVA AI team can see where users drop off during install.
...
#
ANALYTICS_HOST="${NOVA_AI_ANALYTICS_HOST:-https://34.231.106.201.sslip.io}"
ANALYTICS_KEY="${NOVA_AI_ANALYTICS_KEY:-phc_ysKu72QaxzYNmDpHFcesD2ZZAe68zkdWJEKoYYkc5e3n}"
ANON_ID_FILE="$NOVA_AI_HOME/anon_id"
...
clone_repo() {
    ...
    git clone --depth 1 "$NOVA_AI_REPO_URL" "$SRC_DIR"
}
...
write_config() {
    "$VENV_DIR/bin/nova" _bootstrap --write-config \
...
install_symlinks() {
    ...
    ln -sf "$SCRIPTS_DIR/nova-wrapper.sh" "$HOME/.local/bin/nova"
...
ensure_path() {
    ...
    if grep -q "NOVA AI" "$rc" 2>/dev/null; then
...
from nova_ai.core.config import detect_hardware, recommend_model
hw = detect_hardware()
...
# ---- run ----
echo "NOVA AI installer"
echo "  install dir: $NOVA_AI_HOME"
echo "  WSL2:        $WSL"
echo
...
step clone_repo         "Clone NOVA AI repo" clone_repo
step copy_scripts       "Copy install scripts"  copy_scripts
step create_venv        "Create venv"           create_venv
step editable_install   "Install NOVA AI"    editable_install
...
#
#   PATH ok + model ok   -> "type nova"
#   PATH new + model ok  -> "source rc && nova  (or open new terminal)"
#   PATH ok + model bad  -> "model still downloading; nova doctor"
#   PATH new + model bad -> "source rc && nova doctor; chat works once download finishes"
#
# `nova` and `nova doctor` need PATH equally, so the source/restart
# guidance goes first when PATH was modified.
NEXT_CMD="nova"
if [[ "$MODEL_PULL_OK" -ne 1 ]]; then
    NEXT_CMD="nova doctor"
fi
...
if [[ "$MODEL_PULL_OK" -ne 1 ]]; then
    cat <<EOF
NOTE: the qwen3.5:2b model didn't finish downloading. 'nova doctor'
...
Background work continues silently:
  ...
  Run 'nova doctor' to check status anytime.
EOF
