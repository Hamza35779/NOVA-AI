...
prove NOVA AI wiring; this validates that a real H100/H200 host can mine
through Pearl and serve inference through NOVA AI.
...
- Docker container `nova_ai-pearl-miner` starts.
- `~/.nova_ai/runtime/mining.json` is written.
...
```bash
docker ps --filter name=nova_ai-pearl-miner
cat ~/.nova_ai/runtime/mining.json
uv run nova mine logs --tail 200
...
## Verify NOVA AI Inference Uses Mining Endpoint

Run:

```bash
uv run nova mine doctor
uv run nova ask "Say hello in one sentence."
```
...
- If metric names differ, attach `/tmp/pearl-gateway-metrics.txt` to the PR and
  update `src/nova_ai/mining/_metrics.py`.
...
```bash
uv run nova mine stop
docker ps --filter name=nova_ai-pearl-miner
test ! -e ~/.nova_ai/runtime/mining.json
```
...
- Container stops.
- Sidecar is removed.
- `nova ask` no longer routes through `vllm-pearl-mining` unless another
...
- NOVA AI inference succeeds through `vllm-pearl-mining`.
...
```bash
uv run nova mine doctor
uv run nova mine status || true
uv run nova mine logs --tail 300 || true
docker inspect nova_ai-pearl-miner || true
