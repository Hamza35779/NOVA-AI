...
The `nova_ai.mining` subsystem follows the same registry pattern as engines,
agents, tools, memory, and channels. New mining paths should be provider
modules, not special cases in the CLI or engine layer.

## Provider Contract

Every provider implements `nova_ai.mining.MiningProvider`:
...
The runtime sidecar lives at `~/.nova_ai/runtime/mining.json`. Engine
...
- `MiningStats`
- `Sidecar`
- `nova mine doctor` capability iteration
