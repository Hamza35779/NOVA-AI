...
2. Check if a [GitHub issue](https://github.com/Hamza35779/NOVA-AI/issues) already exists for it — if not, [open one](https://github.com/Hamza35779/NOVA-AI/issues/new/choose)
3. Comment **"take"** on the issue to get auto-assigned
4. Read the [Contributing Guide](https://github.com/Hamza35779/NOVA-AI/blob/main/CONTRIBUTING.md) for development setup and PR process
...
|-----|---------|---------------------|
| **Ready** | Well-scoped, implementation path is clear | Pick it up — check [issues](https://github.com/Hamza35779/NOVA-AI/issues) for a spec or write one |
| **Design Needed** | Concept is clear but needs a spec before code | Start a [design discussion](https://github.com/Hamza35779/NOVA-AI/discussions) or draft an RFC |
...
Operators are NOVA AI's key differentiator — persistent, scheduled, stateful agents that run autonomously on personal devices. The current tick-based architecture (OperatorManager → TaskScheduler → AgentExecutor → OperativeAgent) is solid but needs hardening for truly long-horizon autonomy.

#### Where you can help

| Item | Maturity | Details |
|------|----------|---------|
| Operator health checks & heartbeat monitoring | **Shipped** | Done — `OperatorManager.health()` derives healthy/degraded/failing/stalled/paused labels from task state and run logs; surfaced via `nova operators status`. |
| Metrics collection for operator manifests | **Shipped** | Done — `OperatorManager.collect_metrics()` resolves manifest-declared metrics against the TelemetryAggregator. |
| Capability policy enforcement | **Shipped** | Done — `activate()` checks `required_capabilities` against the RBAC `CapabilityPolicy` and raises `OperatorCapabilityError` on denial. |
| Rate limiting per operator | **Shipped** | Done — `rate_limit_rpm` manifest field; the scheduler skips early ticks and pushes `next_run` past the minimum interval. A `max_consecutive_failures` circuit breaker auto-pauses repeat failures. |
...
Personal AI must be accessible from the devices people actually carry. NOVA AI runs on laptops, workstations, and servers — users interact via their phones.
...
NOVA AI has reference docs and four tutorials, but critical gaps remain in continuous agents, LM evaluation, learning approaches, and custom tools. Video tutorials are scoped as a contributor opportunity — written tutorials come first, with video scripts included so anyone can record.
