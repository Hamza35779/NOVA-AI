...
NOVA AI on Windows installs two ways: **WSL2** (this page — the
...
- The first time you run `nova`, the WSL kernel may show a "process running in background" notification — that's the bg-orchestrator detaching. It's expected.
- Models are stored in WSL's filesystem (`~/.nova_ai/`), not your Windows drive. To free up space later: `nova-uninstall` removes everything.
