---
title: Configuration
description: Complete reference for NOVA AI configuration
---

# Configuration

NOVA AI uses a TOML configuration file to control engine selection, model identity, memory backends, agent behavior, and more. This page is the complete reference for every configuration option, organized by primitive.
...
NOVA AI creates the `~/.nova_ai/` directory and populates it with a default config when you run `nova init`.

## Relocating the NOVA AI directory

NOVA AI keeps **all** of its state — config, databases, caches, logs,
...
`~/.nova_ai`, but you can move it.

The root is resolved in priority order:

1. **`$NOVA_AI_HOME`** — explicit override. Honored by both the installer
   and the Python runtime.
2. **`$XDG_DATA_HOME/nova_ai`** — used when `$XDG_DATA_HOME` is set (a single
   `nova_ai` directory nested under it, per the XDG Base Directory spec).
3. **`~/.nova_ai`** — the default. With no environment variables set, the
...
!!! note "Migration"
    ...
    existing installs. If you set `NOVA_AI_HOME` (or `XDG_DATA_HOME`) on a
    machine that already has data in `~/.nova_ai`, NOVA AI will look in
    the new location and not see your old data — move it yourself if you want
    to keep it: `mv ~/.nova_ai "$NOVA_AI_HOME"`.

`$NOVA_AI_CONFIG` still points at an explicit `config.toml` file
...
3. Writes `~/.nova_ai/config.toml` with sensible defaults
...
|-------|------|---------|-------------|
| `default` | string | Auto-detected | Default engine backend. One of: `ollama`, `vllm`, `llamacpp`, `sglang`, `cloud`. Set automatically by `nova init` based on hardware detection. |
...
!!! tip "Engine fallback"
    If the configured default engine is unreachable, NOVA AI automatically probes all registered engines and falls back to any healthy one.
...
```bash
nova ask --router heuristic "Hello"
```
...
| `db_path` | string | `~/.nova_ai/memory.db` | Path to the SQLite memory database. Used by the `sqlite` backend. |
...
Controls the OpenAI-compatible API server started by `nova serve`.
...
```bash
nova serve --host 127.0.0.1 --port 9000 --model qwen3:8b --agent simple
```
...
[traces]
enabled = false
db_path = "~/.nova_ai/traces.db"
```

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled` | bool | `false` | Whether to record traces for each agent interaction. |
| `db_path` | string | `~/.nova_ai/traces.db` | Path to the SQLite trace database. |
...
| `skills_dir` | string | `~/.nova_ai/skills/` | Directory where skills are installed. |
...
| `overlay_dir` | string | `~/.nova_ai/learning/skills/` | Where optimized skill overlays are stored. |
...
When you run `nova init`, NOVA AI probes your system to detect available hardware. The detection runs in this order:
...
If you have an existing `~/.nova_ai/config.toml` from a previous version, here is what changed and how to update it.
...
You can configure NOVA AI entirely from Python without a TOML file:
...
```python
j = Nova(config_path="/path/to/my-config.toml")
```
...
NOVA AI respects the following environment variables:
