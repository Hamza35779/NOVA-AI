---
title: Installation
description: Get NOVA AI running — browser app, desktop app, CLI, or Python SDK
search:
  boost: 3
---
...
NOVA AI runs entirely on your hardware. Choose the interface that fits your workflow.
...
The desktop app is a native window for the NOVA AI chat UI. All inference and backend
...
The command-line interface is the fastest way to interact with NOVA AI
...
For programmatic access, the `Nova` class provides a high-level sync API.
...
NOVA AI uses optional extras to keep the base installation lightweight.
...
!!! note "Ollama, vLLM, and llama.cpp are HTTP-based"
    These engines have no additional Python dependencies — NOVA AI communicates over HTTP. You still need the engine software running on your machine.
...
| `server` | `uv sync --extra server` | OpenAI-compatible API server (`nova serve`) |
...
NOVA AI requires at least one inference backend. Choose the one that matches your hardware.
