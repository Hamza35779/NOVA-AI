...
!!! tip "Prerequisites"
    ...
    - NOVA AI installed: run `uv sync --extra dev` from the repository root
...
The script creates a `Nova` instance and delegates the research task to an `OrchestratorAgent` with five tools wired in. The orchestrator iterates through multiple tool-calling turns, deciding at each step whether to search, store, think, or synthesize.

```mermaid
sequenceDiagram
    participant U as User
    participant J as Nova SDK
...
j = Nova(model="qwen3:8b", engine_key="ollama")  # (1)!
try:
    response = j.ask(
...
1. Creates a `Nova` instance targeting the local Ollama engine with `qwen3:8b`. Both parameters are optional — omitting them uses auto-detected defaults from `~/.nova_ai/config.toml`.
...
from nova_ai.recipes import load_recipe
from nova_ai import SystemBuilder
...
This is useful when you want to version-control the research configuration, share it with collaborators, or feed it to the `nova eval` runner for benchmarking.
...
Run `nova agent info orchestrator` to see the full tool catalog.
...
- [Getting Started: Configuration](../getting-started/configuration.md) — how to configure engines and models in `~/.nova_ai/config.toml`
