...
```bash
python examples/nova_ai/spec_search_quickstart.py
```
...
`nova_ai.learning.spec_search.composite_reward.score_batch`.
...
`configs/nova_ai/examples/spec-search-quickstart.toml`. Copy it to
`~/.nova_ai/config.toml` (or set `NOVA_AI_CONFIG` to it) and the
...
| `trace_store` | `MagicMock` | `nova_ai.traces.store.TraceStore(home / "traces.db")` |
| `student_runner` | `MagicMock` | `nova_ai.learning.spec_search.student_runner.VLLMStudentRunner(host=..., model=...)` |
| `judge` | `MagicMock` | `nova_ai.evals.core.scorer.LLMJudgeScorer(...)` (or a deterministic scorer if your benchmark provides one) |
| `session_store` | `MagicMock` | `nova_ai.learning.spec_search.storage.session_store.SessionStore(home / "learning" / "sessions.db")` |
| `checkpoint_store` | `MagicMock` | `nova_ai.learning.spec_search.checkpoint.store.CheckpointStore(home / "learning" / "checkpoints")` |
...
1. Create `src/nova_ai/evals/datasets/<corpus>.py` implementing
   `DatasetProvider` (`adp.py` is a small reference). The provider's
   `load(max_samples, seed, split)` must respect `split` via
   `apply_split` from `nova_ai.evals.core.splits`.
2. Register: `@DatasetRegistry.register("<corpus>")`.
3. Feed it to the proposer via the trace store:

```python
from nova_ai.evals.datasets.adp import ADPDataset
from nova_ai.learning.spec_search.external_adapter import (
...
`src/nova_ai/evals/backends/nova_agent.py` previously hardcoded
`builder.telemetry(telemetry).traces(True).build()`, ignoring the
`telemetry` parameter. This silently caused every agent-backend
evaluation to write to `~/.nova_ai/traces.db` regardless of caller
...
- `examples/nova_ai/spec_search_quickstart.py` — runnable end-to-end demo.
- `configs/nova_ai/examples/spec-search-quickstart.toml` — prebuilt config.
- `src/nova_ai/learning/spec_search/orchestrator.py` — `SpecSearchOrchestrator` (single session).
- `src/nova_ai/learning/spec_search/multi_session.py` — `SpecSearchLoop` (Algorithm 1).
- `src/nova_ai/learning/spec_search/composite_reward.py` — paper Eq. 1.
- `src/nova_ai/learning/spec_search/gate/benchmark_gate.py` — `GateOK` predicate.
- `src/nova_ai/learning/spec_search/external_adapter.py` — corpus → trace adapter.
