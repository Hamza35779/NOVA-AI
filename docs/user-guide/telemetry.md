...
NOVA AI has two complementary observability systems: **telemetry** for per-inference metrics and **traces** for full interaction-level recording. Together, they provide comprehensive insight into system behavior and power the learning system's routing policy updates.
...
bus = EventBus()
store = TelemetryStore(db_path="~/.nova_ai/telemetry.db")
...
agg = TelemetryAggregator(db_path="~/.nova_ai/telemetry.db")
...
nova telemetry stats -n 5          # Top 5 models only

# Export records
nova telemetry export              # JSON to stdout
nova telemetry export -f csv       # CSV to stdout
nova telemetry export -o data.json # JSON to file
nova telemetry export -f csv -o metrics.csv

# Clear all records
nova telemetry clear               # With confirmation prompt
nova telemetry clear --yes         # Without confirmation
```
