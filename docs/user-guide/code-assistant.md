...
This writes a pre-configured `~/.nova_ai/config.toml` for the code assistant.
...
```bash
nova ask "Write a Python script that reads a CSV file and prints the top 5 rows"
```
...
nova ask "Write a Python script that parses JSON from stdin"

# Read and explain existing code
nova ask "Read main.py and explain the architecture"

# Fix a bug
nova ask "Find and fix the bug in test_utils.py"

# Run tests
nova ask "Run the test suite and summarize any failures"

# Explicitly specify agent and tools
nova ask --agent orchestrator --tools code_interpreter "Calculate the first 20 Fibonacci numbers"
...
The preset writes this to `~/.nova_ai/config.toml`:
...
```bash
# Write a new script
nova ask "Write a Python script that converts YAML to JSON"

# Explain existing code
nova ask "Read src/nova_ai/core/events.py and explain the EventBus pattern"

# Debug a failing test
nova ask "Run pytest tests/test_memory.py -v and fix any failures"

# Refactor code
nova ask "Read utils.py and refactor the parse_config function to use dataclasses"

# Generate tests
nova ask "Read src/nova_ai/tools/calculator.py and write unit tests for it"

# Shell tasks
nova ask "Find all Python files larger than 100KB in this repo"
```
...
- The agent asks for confirmation before executing potentially destructive commands when running in interactive mode (`nova chat`).
- For stronger isolation, use the sandboxed agent: `nova ask --agent sandboxed --tools code_interpreter "..."`, which runs inside a Docker/Podman container.
...
**Shell command fails** -- The `shell_exec` tool runs commands relative to where you launched `nova`. Use `cd /path && command` in your prompt if needed, or run `nova` from the project directory.
