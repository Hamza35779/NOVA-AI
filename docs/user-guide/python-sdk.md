...
The NOVA AI Python SDK provides a high-level interface for interacting with local inference engines, managing memory, and running agent workflows. The primary entry point is the `Nova` class.
...
|---------------|------------------|---------|----------------------------------------------------------------|
| `config`      | `NovaConfig`   | `None`  | Provide a pre-built configuration object                       |
...
If no `config` or `config_path` is provided, the SDK loads configuration from the default location (`~/.nova_ai/config.toml`), falling back to built-in defaults.
...
|-----------|----------------|-----------------------------------|
| `config`  | `NovaConfig` | The active configuration object   |
| `version` | `str`          | The NOVA AI version string     |
...
The `Nova.memory` attribute provides a `MemoryHandle` for document indexing, search, and statistics. The memory backend is lazily initialized on first use.
...
Release all resources held by the `Nova` instance, including the memory backend, telemetry store, and engine connection.
...
!!! tip "Context Manager Pattern"
    While `Nova` does not implement `__enter__`/`__exit__` directly, you should always call `close()` when done to free database connections and other resources:
