...
- **Persistence:** SQLite database file (default: `~/.nova_ai/memory.db`)
...
```python
from nova_ai.tools.storage.bm25 import BM25Memory
...
!!! note "Backward compatibility"
    The old `from nova_ai.memory.bm25 import BM25Memory` still works via backward-compatibility shims, but new code should use the canonical `nova_ai.tools.storage.*` imports.
...
[Source: docs/intro.md] NOVA AI is a modular AI framework...
...
nova memory index ./notes/ --chunk-size 256 --chunk-overlap 32

# Search the memory store
nova memory search "machine learning"
