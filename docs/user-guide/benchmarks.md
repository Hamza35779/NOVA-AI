...
NOVA AI ships with two benchmarks:
...
from nova_ai.bench._stubs import BenchmarkResult
...
from nova_ai.bench.latency import LatencyBenchmark
from nova_ai.bench.throughput import ThroughputBenchmark
...
nova bench run -b throughput -n 20

# Specify model and engine
nova bench run -m qwen3:8b -e ollama

# Output JSON summary to stdout
nova bench run --json

# Write JSONL results to a file
nova bench run -o results.jsonl

# Combine options
nova bench run -b latency -n 100 -m qwen3:8b --json -o latency.jsonl
```
...
```python
import time
from nova_ai.bench._stubs import BaseBenchmark, BenchmarkResult
...
```bash
nova bench run -b context_length
```
