...
| **Overlay** | Sidecar file at `~/.nova_ai/learning/skills/` storing optimized descriptions and few-shot examples |
...
nova skill install hermes:apple-notes

# Bulk install a category
nova skill sync hermes --category research

# Run a skill directly
nova skill run math-solver -a expression="41 + 82"

# See skill details
nova skill info research-and-summarize
```
...
nova skill sync hermes --category research
nova skill sync hermes --category coding
nova skill sync hermes  # everything (~150 skills)
```
...
nova skill install openclaw:0xv4l3nt1n3/etherscan

# Bulk install with search filter
nova skill sync openclaw --search "web3|crypto"
```

### From Any GitHub Repo

```bash
nova skill install github:user/repo/path/to/skill --url https://github.com/user/repo
```
...
```bash
nova skill install github:Xquik-dev/hermes-tweet/skills/hermes-tweet --url https://github.com/Xquik-dev/hermes-tweet
```

### Config-Driven Auto Import

Add sources to `~/.nova_ai/config.toml` for automatic syncing:
...
NOVA AI can automatically mine your trace history for recurring tool sequences and surface them as candidate skills:

```bash
# Preview discovered patterns without writing
nova skill discover --dry-run --min-frequency 3

# Write discovered skills to ~/.nova_ai/skills/discovered/
nova skill discover
```

Discovered skills land in `~/.nova_ai/skills/discovered/` and automatically appear in `nova skill list` on the next session.
...
nova optimize skills --dry-run

# Run DSPy optimization
nova optimize skills --policy dspy --min-traces 3

# Run GEPA evolutionary optimization
nova optimize skills --policy gepa --min-traces 3

# Inspect what optimization produced
nova skill show-overlay research-and-summarize
```

Optimization results are stored as sidecar overlays at `~/.nova_ai/learning/skills/<skill-name>/optimized.toml`. They override the skill's description and add few-shot examples to the agent's system prompt. The original skill files are never modified.
...
# Single condition
nova bench skills --condition skills_optimized_dspy
```
...
|------|--------|-------------|---------|
| **Bundled** | Ships with NOVA AI | Implicit trust | Full access within declared capabilities |
...
```bash
nova skill install hermes:arxiv --with-scripts
```
...
skills_dir = "~/.nova_ai/skills/"  # where skills are installed
...
overlay_dir = "~/.nova_ai/learning/skills/"
```
...
2. **User** `~/.nova_ai/skills/`
3. **Bundled** (shipped with NOVA AI)
...
| `nova skill info <name>` | Show detailed skill information |
| `nova skill run <name> [-a key=value]` | Execute a skill directly |
| `nova skill install <source>:<name>` | Install from Hermes, OpenClaw, or GitHub |
| `nova skill sync [<source>] [--category C]` | Bulk install + update from sources |
| `nova skill sources` | List configured skill sources |
| `nova skill update` | Pull latest from configured sources |
| `nova skill remove <name>` | Remove an installed skill |
| `nova skill search <query>` | Search the skill index |
| `nova skill discover [--dry-run]` | Mine traces for recurring tool patterns |
| `nova skill show-overlay <name>` | Inspect optimization output for a skill |
| `nova optimize skills [--policy dspy\|gepa]` | Optimize skill descriptions + few-shot examples |
| `nova bench skills [--condition C]` | Run the PinchBench skills benchmark |
