...
This page traces the end-to-end journey of a user query through the NOVA AI system, from the moment it enters the CLI or SDK to the final response and telemetry recording.
...
sequenceDiagram
    ...
    User->>CLI: nova ask "query" / j.ask("query")
    CLI->>CFG: load_config()
    CFG-->>CLI: NovaConfig (hardware, engine defaults)
...
NOVA AI supports two query processing paths, selected by the `--agent` CLI flag or the `agent` parameter in the SDK.
...
```bash
# CLI
nova ask --agent orchestrator --tools calculator,think "What is 2^10 + 3^5?"
...
```python
config = load_config()  # Reads ~/.nova_ai/config.toml
```
...
- Returns a `NovaConfig` dataclass with all settings
...
```python
from nova_ai.learning import ensure_registered
...
The `TelemetryStore` subscribes to `TELEMETRY_RECORD` events on the EventBus and writes records to `~/.nova_ai/telemetry.db`.
...
The `Nova` class in `sdk.py` provides the same query flow through a Python API:
