...
description: The five-primitive architecture behind NOVA AI — Intelligence, Engine, Agents, Tools, and Learning
search:
  boost: 2
---
...
NOVA AI is a research framework for studying on-device AI systems. Its architecture is organized around **five core abstractions** -- Intelligence, Engine, Agentic Logic, Memory, and Learning -- that work together through trace-driven feedback.

![NOVA AI Architecture](../assets/NovaAI_Architecture.png)
...
The sandbox module (`nova_ai.sandbox`) adds a `SandboxedAgent` wrapper that runs any `BaseAgent` inside a Docker or Podman container with mount-security enforcement, and a `ContainerRunner` that manages the container lifecycle.
...
All extensible components in NOVA AI use a **decorator-based registry** for runtime discovery. The pattern is implemented in `RegistryBase[T]`, a generic base class that provides isolated storage per typed subclass.
...
src/nova_ai/
    core/               Core infrastructure shared by all primitives
        ...
        config.py           NovaConfig, hardware detection, TOML loading
    ...
    cli/                CLI commands (Click-based)
        ask.py              nova ask -- query the assistant
        serve.py            nova serve -- start API server

    sdk.py              Nova class -- high-level Python SDK
