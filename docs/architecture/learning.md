...
!!! note "Backward compatibility"
    The canonical locations are now `nova_ai.learning._stubs` (for `RouterPolicy` and `QueryAnalyzer`) and `nova_ai.core.types` (for `RoutingContext`). The old `nova_ai.intelligence._stubs` import path still works via a backward-compatibility shim, but new code should import from `nova_ai.learning._stubs`.
...
```bash
nova ask --router learned "What is the capital of France?"
```
...
```python
from nova_ai.learning.router import HeuristicRouter, build_routing_context
...
```python
from nova_ai.learning.router import build_routing_context
...
```python
from nova_ai.learning.trace_policy import TraceDrivenPolicy
from nova_ai.traces.analyzer import TraceAnalyzer
...
```python
from nova_ai.learning.sft_policy import SFTRouterPolicy
# or via the backward-compat alias:
from nova_ai.learning.sft_policy import SFTPolicy
```
...
```python
from nova_ai.learning.agent_advisor import AgentAdvisorPolicy
```
...
```python
from nova_ai.learning.icl_updater import ICLUpdaterPolicy
```
...
```python
from nova_ai.learning.heuristic_reward import HeuristicRewardFunction
...
store = TraceStore("~/.nova_ai/traces.db")
...
over NOVA AI's 5-primitive configuration space. It automates finding optimal
...
The optimization framework has full Rust parity via the `nova_ai-learning`
...
`nova_ai.memory.consolidation` turns episodic traces into durable knowledge: `SessionMiner` clusters recent traces by query class (optionally refined by embedding similarity), a local LLM distills each cluster into atomic facts (`{content, topic, confidence}`), and `FactStore` (`~/.nova_ai/learning/consolidation/facts.db`) deduplicates them, resolves contradictions by confidence (winner supersedes; unconfident pairs coexist), and decays facts untouched for `decay_days`. Active facts render as a bounded core-memory block (`memory.consolidation.inject`) prepended to every query by `QueryOrchestrator.ask` — fail-open: an empty or broken fact base never breaks answering. Off by default; `nova memory consolidate forget` is the manual override. See [Memory Consolidation](../learning/memory-consolidation.md).

### Skill Foundry (synthesize skills from repeated tool runs)

`nova_ai.learning.skillforge` turns repeated multi-step tool workflows into skills: `PatternMiner` groups `TOOL_CALL` trace steps by ordered sequence (≥ `min_pattern_count` repetitions, feedback ≥ `min_feedback`), `SkillSynthesizer` has the local LLM write a `skill.toml` constrained to the real `ToolRegistry` catalog and the observed call order, and `run_gauntlet` verifies the candidate in three gates — static (name/capabilities/unknown-tool rejection), replay (mined examples through `SkillExecutor`, code/shell steps forced through the subprocess sandbox), and judge (LLM vs the trace's known-good result). Adoption writes `~/.nova_ai/skills/generated/<name>/skill.toml` with a provenance block — discovered automatically, reversible with `nova forge revert`. Off by default; `nova forge adopt` is the only mutation. See [Skill Foundry](../learning/skill-foundry.md).
