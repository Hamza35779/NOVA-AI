...
NOVA AI follows a set of design principles that guide every architectural decision. These principles ensure the framework remains extensible, portable, and easy to work with.

---

## 1. Pluggable Everything

Every major component in NOVA AI is defined as an **abstract base class** (ABC) with concrete implementations registered at runtime. This means you can swap, extend, or replace any part of the system without modifying existing code.
...
NOVA AI is designed to work **entirely without network access**. All core functionality -- inference, memory, agents, tools, telemetry -- operates locally. Cloud APIs are optional extensions, never requirements.
...
j = Nova(engine_key="ollama")  # Local Ollama server
...
NOVA AI **auto-detects system hardware** at startup and recommends the optimal inference engine. The `detect_hardware()` function probes:
...
This recommendation is written to `config.toml` during `nova init` and used as the default engine:

```bash
nova init --force
# Detects hardware, writes ~/.nova_ai/config.toml with:
# [engine]
...
```bash
nova telemetry stats          # Aggregated statistics
nova telemetry export --json  # Export all records
```
...
NOVA AI provides a **clean Python API** through the `Nova` class. There is no framework lock-in -- the SDK is a standard Python package with dataclass-based types and no required web framework.
...
- **Optional dependencies** via extras (`nova_ai[server]`, `nova_ai[memory-colbert]`, etc.)
...
The API server (`nova serve`) implements the **OpenAI chat completions API format**, making NOVA AI a drop-in replacement for OpenAI in existing applications.
...
Any OpenAI client library can connect to NOVA AI:
...
NOVA AI requires **no external services** for core functionality. Everything needed to run the system is included or uses standard system libraries.
...
The only external requirement is a running inference engine (Ollama, vLLM, etc.), which is the model server itself -- not a dependency of NOVA AI.

Optional features that require additional packages:

| Feature | Extra | Packages |
|---------|-------|----------|
| FAISS memory | `nova_ai[memory-faiss]` | `faiss-cpu`, `sentence-transformers` |
| ColBERT memory | `nova_ai[memory-colbert]` | `colbert-ai`, `torch` |
| BM25 memory | `nova_ai[memory-bm25]` | `rank-bm25` |
| API server | `nova_ai[server]` | `fastapi`, `uvicorn` |
| Cloud inference | `nova_ai[inference-cloud]` | `openai`, `anthropic`, `google-genai` |
| vLLM engine | `nova_ai[inference-vllm]` | `vllm` |
| PDF ingestion | `nova_ai[memory-pdf]` | `pdfplumber` |
| WhatsApp Baileys | `nova_ai[channel-whatsapp-baileys]` | Node.js 22+ |
