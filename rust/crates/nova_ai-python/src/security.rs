...
pub struct PySecretScanner {
    inner: nova_ai_security::SecretScanner,
}
...
impl PySecretScanner {
    ...
    fn new() -> Self {
        Self {
            inner: nova_ai_security::SecretScanner::new(),
        }
    }
...
pub struct PyPIIScanner {
    inner: nova_ai_security::PIIScanner,
}
...
impl PyPIIScanner {
    ...
    fn new() -> Self {
        Self {
            inner: nova_ai_security::PIIScanner::new(),
        }
    }
...
pub struct PyGuardrailsEngine {
    inner: nova_ai_security::GuardrailsEngine<nova_ai_engine::Engine>,
}
...
impl PyGuardrailsEngine {
    ...
    fn new(
        ...
        let redaction_mode = match mode {
            "redact" => nova_ai_security::RedactionMode::Redact,
            "block" => nova_ai_security::RedactionMode::Block,
            _ => nova_ai_security::RedactionMode::Warn,
        };
        Ok(Self {
            inner: nova_ai_security::GuardrailsEngine::new(
                engine,
                redaction_mode,
                scan_input,
                scan_output,
    ...
    fn engine_id(&self) -> &str {
        use nova_ai_engine::InferenceEngine;
...
pub struct PyAuditLogger {
    inner: parking_lot::Mutex<nova_ai_security::AuditLogger>,
}
...
impl PyAuditLogger {
    ...
    fn new(path: Option<&str>) -> PyResult<Self> {
        ...
        let inner = nova_ai_security::AuditLogger::new(&db_path)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
        Ok(Self {
...
pub struct PyCapabilityPolicy {
    inner: nova_ai_security::CapabilityPolicy,
}
...
impl PyCapabilityPolicy {
    ...
    fn new(default_deny: bool) -> Self {
        Self {
            inner: nova_ai_security::CapabilityPolicy::new(default_deny),
        }
    }
...
pub struct PyInjectionScanner {
    inner: nova_ai_security::InjectionScanner,
}
...
impl PyInjectionScanner {
    ...
    fn new() -> Self {
        Self {
            inner: nova_ai_security::InjectionScanner::new(),
        }
    }
...
pub struct PyRateLimiter {
    inner: nova_ai_security::RateLimiter,
}
...
impl PyRateLimiter {
    ...
    fn new(requests_per_minute: u32, burst_size: u32) -> Self {
        Self {
            inner: nova_ai_security::RateLimiter::new(
                nova_ai_security::RateLimitConfig {
                    requests_per_minute,
                    burst_size,
                    enabled: true,
                },
...
pub struct PyTaintSet {
    inner: nova_ai_security::TaintSet,
}
...
impl PyTaintSet {
    ...
    fn new() -> Self {
        Self {
            inner: nova_ai_security::TaintSet::new(),
        }
    }

    fn add(&mut self, label: &str) {
        let taint_label = match label {
            "pii" => nova_ai_security::TaintLabel::Pii,
            "secret" => nova_ai_security::TaintLabel::Secret,
            "user_private" => nova_ai_security::TaintLabel::UserPrivate,
            "external" => nova_ai_security::TaintLabel::External,
            _ => nova_ai_security::TaintLabel::External,
        };
        // TaintSet is immutable-style; union with a single-label set.
        self.inner = self.inner.union(
            &nova_ai_security::TaintSet::from_labels(&[taint_label]),
        );
    }
