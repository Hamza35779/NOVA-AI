...
pub struct PyTraceStore {
    pub inner: Arc<nova_ai_traces::TraceStore>,
}
...
impl PyTraceStore {
    ...
    fn new(path: Option<&str>) -> PyResult<Self> {
        let inner = match path {
            Some(p) => nova_ai_traces::TraceStore::new(std::path::Path::new(p)),
            None => nova_ai_traces::TraceStore::in_memory(),
        }
...
pub struct PyTraceCollector {
    inner: nova_ai_traces::TraceCollector,
}
...
impl PyTraceCollector {
    ...
    fn new(store: &PyTraceStore) -> Self {
        Self {
            inner: nova_ai_traces::TraceCollector::new(Arc::clone(&store.inner)),
        }
    }
...
pub struct PyTraceAnalyzer {
    store: Arc<nova_ai_traces::TraceStore>,
}
...
impl PyTraceAnalyzer {
    ...
    fn stats(&self) -> PyResult<String> {
        let analyzer = nova_ai_traces::TraceAnalyzer::new(&self.store);
        let stats = analyzer
            .overall_stats()
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
