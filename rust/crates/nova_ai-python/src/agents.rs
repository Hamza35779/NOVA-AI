...
use crate::RUNTIME;
use nova_ai_agents::OjAgent;
use nova_ai_engine::rig_adapter::RigModelAdapter;
use nova_ai_engine::Engine;
use pyo3::prelude::*;
use std::sync::Arc;
...
enum AgentEnum {
    Simple(nova_ai_agents::SimpleAgent<DefaultAdapter>),
    Orchestrator(nova_ai_agents::OrchestratorAgent<DefaultAdapter>),
    NativeReAct(nova_ai_agents::NativeReActAgent<DefaultAdapter>),
}
...
fn make_adapter(engine_key: &str, model: &str) -> PyResult<DefaultAdapter> {
    let config = nova_ai_core::NovaConfig::default();
    let engine = nova_ai_engine::get_engine_static(&config, Some(engine_key))
...
impl PySimpleAgent {
    ...
    fn new(
        ...
        let agent = nova_ai_agents::SimpleAgent::new(adapter, system_prompt, temperature);
...
impl PyOrchestratorAgent {
    ...
    fn new(
        ...
        let adapter = make_adapter(engine_key, model)?;
        let executor = Arc::new(nova_ai_tools::ToolExecutor::new(None, None));
        let agent = nova_ai_agents::OrchestratorAgent::new(
...
impl PyNativeReActAgent {
    ...
    fn new(
        ...
        let adapter = make_adapter(engine_key, model)?;
        let executor = Arc::new(nova_ai_tools::ToolExecutor::new(None, None));
        let agent = nova_ai_agents::NativeReActAgent::new(
...
impl PyNativeOpenHandsAgent {
    ...
    fn new(
        ...
        let engine = nova_ai_engine::get_engine_static(&config, Some(engine_key))
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
        let adapter = nova_ai_engine::rig_adapter::RigModelAdapter::new(
            Arc::new(engine),
            model.to_string(),
        );
        let executor = Arc::new(nova_ai_tools::ToolExecutor::new(None, None));
        let agent = nova_ai_agents::NativeOpenHandsAgent::new(
            adapter,
            executor,
            max_turns,
            temperature,
...
impl PyMonitorOperativeAgent {
    ...
    fn new(
        ...
        let mem_ext = match memory_extraction {
            "scratchpad" => nova_ai_agents::MemoryExtraction::Scratchpad,
            "structured_json" => nova_ai_agents::MemoryExtraction::StructuredJson,
            "none" => nova_ai_agents::MemoryExtraction::None,
            _ => nova_ai_agents::MemoryExtraction::CausalityGraph,
        };
        let obs_comp = match observation_compression {
            "truncate" => nova_ai_agents::ObservationCompression::Truncate,
            "none" => nova_ai_agents::ObservationCompression::None,
            _ => nova_ai_agents::ObservationCompression::Summarize,
        };
        let ret_strat = match retrieval_strategy {
            "keyword" => nova_ai_agents::RetrievalStrategy::Keyword,
            "semantic" => nova_ai_agents::RetrievalStrategy::Semantic,
            "none" => nova_ai_agents::RetrievalStrategy::None,
            _ => nova_ai_agents::RetrievalStrategy::HybridWithSelfEval,
        };
        let task_dec = match task_decomposition {
            "monolithic" => nova_ai_agents::TaskDecomposition::Monolithic,
            "hierarchical" => nova_ai_agents::TaskDecomposition::Hierarchical,
            _ => nova_ai_agents::TaskDecomposition::Phased,
        };

        let monitor_config = nova_ai_agents::MonitorConfig {
        ...
        let agent = nova_ai_agents::MonitorOperativeAgent::new(
            adapter,
            executor,
            max_turns,
            temperature,
...
pub struct PyLoopGuard {
    inner: nova_ai_agents::LoopGuard,
}
...
impl PyLoopGuard {
    ...
    fn new(max_identical: usize, max_ping_pong: usize, poll_budget: usize) -> Self {
        Self {
            inner: nova_ai_agents::LoopGuard::new(max_identical, max_ping_pong, poll_budget),
        }
    }
