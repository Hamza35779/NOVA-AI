...
pub struct PyWorkflowGraph {
    inner: nova_ai_workflow::WorkflowGraph,
}
...
impl PyWorkflowGraph {
    ...
    fn new(name: &str) -> Self {
        Self {
            inner: nova_ai_workflow::WorkflowGraph::new(name),
        }
    }
    ...
    fn add_node(
        ...
        let nt = match node_type {
            "tool" => nova_ai_workflow::NodeType::Tool,
            "condition" => nova_ai_workflow::NodeType::Condition,
            "parallel" => nova_ai_workflow::NodeType::Parallel,
            "loop" => nova_ai_workflow::NodeType::Loop,
            "transform" => nova_ai_workflow::NodeType::Transform,
            _ => nova_ai_workflow::NodeType::Agent,
        };
        let node = nova_ai_workflow::WorkflowNode {
            id: id.to_string(),
            node_type: nt,
    ...
    fn add_edge(&mut self, source: &str, target: &str) -> PyResult<()> {
        let edge = nova_ai_workflow::WorkflowEdge {
...
pub struct PyWorkflowBuilder {
    inner: Option<nova_ai_workflow::WorkflowBuilder>,
}
...
impl PyWorkflowBuilder {
    ...
    fn new(name: &str) -> Self {
        Self {
            inner: Some(nova_ai_workflow::WorkflowBuilder::new(name)),
        }
    }
