...
//!
//! Rust translation of `src/nova_ai/core/config.py`.
...
/// Default config directory: `~/.nova_ai/`
pub fn default_config_dir() -> PathBuf {
    dirs_home().join(".nova_ai")
}

/// Default config file path: `~/.nova_ai/config.toml`
...
fn default_sandbox_image() -> String { "nova_ai-sandbox:latest".into() }
...
fn default_operators_dir() -> String { "~/.nova_ai/operators".into() }
...
pub fn load_config(path: Option<&Path>) -> Result<NovaConfig, ConfigError> {
    ...
    let mut cfg = if config_path.exists() {
        ...
        let mut cfg: NovaConfig = toml::from_str(&content)?;
...
mode = "block"
"#;
        let cfg: NovaConfig = toml::from_str(toml_str).unwrap();
