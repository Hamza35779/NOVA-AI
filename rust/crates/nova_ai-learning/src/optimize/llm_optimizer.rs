...
//!
//! Rust translation of `src/nova_ai/learning/optimize/llm_optimizer.py`.
//!
...
/// Uses an LLM to propose optimal NOVA AI configurations.
///
...
impl LLMOptimizer {
    ...
    pub fn analyze_trial(
        ...
        if let Some(ref backend) = self.backend {
            let prompt = format!(
                "Analyze this NOVA AI evaluation result.\n\n\
                 ## Configuration\n{}\n\n\
                 ## Results\n- accuracy: {accuracy:.4}\n\
    ...
    fn build_initial_prompt(&self) -> String {
        ...
        lines.push("You are optimizing an NOVA AI AI system configuration.".into());
