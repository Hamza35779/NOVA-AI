<#
.SYNOPSIS
    NOVA AI native Windows installer.
...
.DESCRIPTION
    ...
    Steps:
      ...
      5. Clone the NOVA AI repository to $env:LOCALAPPDATA\NOVA AI
...
.NOTES
    ...
    pass `--host 0.0.0.0` AND set $env:NOVA_AI_API_KEY (an
...
if (-not $SkipService -and $env:NOVA_AI_SKIP_SERVICE) { $SkipService = $true }
if (-not $Service     -and $env:NOVA_AI_SERVICE)      { $Service     = $true }
if (-not $Force       -and $env:NOVA_AI_FORCE)        { $Force       = $true }
...
if ($pyMajor -ne 3 -or $pyMinor -lt 10 -or $pyMinor -gt 13) {
    Write-Fail @"
Found Python $pyMajor.$pyMinor at $pythonExe, but NOVA AI requires
...
$installRoot = if ($env:NOVA_AI_HOME) {
    $env:NOVA_AI_HOME
} else {
    Join-Path $env:LOCALAPPDATA 'NOVA AI'
}
...
$repoUrl = if ($env:NOVA_AI_REPO_URL) {
    $env:NOVA_AI_REPO_URL
} else {
...
$modelPullOk = $false
if ($ollamaReady) {
    Write-Info "Pulling qwen3.5:2b (~1.5 GB) so 'nova' works on first run..."
...
# ---------------------------------------------------------------------------
# 9. nova.cmd shim - so bare `nova` works in any new PowerShell
# ---------------------------------------------------------------------------
...
# so the shim is self-locating - moving %LOCALAPPDATA%\NOVA AI won't
...
setlocal
set "SRC=%~dp0..\src"
uv run --project "%SRC%" nova %*
"@
Set-Content -Path $shimPath -Value $shimContent -Encoding ASCII

# Add %LOCALAPPDATA%\NOVA AI\bin to User PATH if it isn't already
...
# the entry as `%LOCALAPPDATA%\NOVA AI\bin` (unexpanded) into User
...
Write-Ok "nova shim installed at $shimPath"

# ---------------------------------------------------------------------------
# 10. Optional: register the scheduled-task service
# ---------------------------------------------------------------------------

$serviceScript = Join-Path $srcDir 'deploy\windows\nova-service.ps1'
...
if ($Service) {
    ...
    if ($isInteractive) {
        $reply = Read-Host "Register NOVA AI as a Windows scheduled task (auto-start at logon, loopback only)? [y/N]"
...
if ($shouldInstallService) {
    ...
    Write-Ok "Scheduled task 'NOVA AI' registered (loopback default)."
}
...
Write-Host "  |   NOVA AI install complete    |" -ForegroundColor Green
Write-Host "  +----------------------------------+" -ForegroundColor Green
Write-Host ""
...
$nextCmd = if ($modelPullOk) { 'nova' } else { 'nova doctor' }

if ($pathNeedsRefresh) {
    Write-Host ""
    Write-Host "  Run it:  open a NEW PowerShell, then: $nextCmd" -ForegroundColor Yellow
    Write-Host "           (the nova shim was added to your User PATH; the"
...
if (-not $modelPullOk) {
    ...
    Write-Host "        'nova doctor' shows progress."
}

if ($shouldInstallService) {
    Write-Host ""
    Write-Host "  Service: schtasks /Query /TN NOVA AI     (status)"
    Write-Host "           powershell -File `"$serviceScript`" uninstall    (remove)"
}
Write-Host ""
Write-Host "  Docs:    https://hamza35779.github.io/NOVA-AI/"
Write-Host ""
