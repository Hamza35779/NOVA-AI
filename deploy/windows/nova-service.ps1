<#
.SYNOPSIS
    Register / unregister the NOVA AI Windows scheduled task.

.DESCRIPTION
    The Windows equivalent of deploy/systemd/nova_ai.service and
    deploy/launchd/com.nova-ai.plist.

    Registers a per-user scheduled task named "NOVA AI" that starts
    `nova serve` at logon and restarts on failure. Loopback default
    ...
    Arguments (install only):
      -InstallRoot <path>  default: %LOCALAPPDATA%\NOVA AI (matches
                           install.ps1's default)
      -ListenHost <addr>   default: 127.0.0.1 (loopback). Set to 0.0.0.0
                           ONLY if you also set $env:NOVA_AI_API_KEY
                           — the server refuses to start unauthenticated
                           on a non-loopback bind.
      -ListenPort <int>    default: 8000

    Usage:
      powershell -ExecutionPolicy Bypass -File nova-service.ps1 install
      powershell -ExecutionPolicy Bypass -File nova-service.ps1 uninstall
      powershell -ExecutionPolicy Bypass -File nova-service.ps1 status
#>
...
function Get-DefaultInstallRoot {
    ...
    if ($env:NOVA_AI_HOME) { return $env:NOVA_AI_HOME }
    return (Join-Path $env:LOCALAPPDATA 'NOVA AI')
}
...
function Install-Task {
    ...
    if (-not (Test-Path $srcDir)) {
        Write-Fail "NOVA AI source not found at $srcDir. Run install.ps1 first."
    }
    ...
    # Mirrors deploy/systemd/nova_ai.service's EnvironmentFile guard.
    $isLoopback = ($ListenHost -eq '127.0.0.1' -or $ListenHost -eq 'localhost')
    if (-not $isLoopback -and -not $env:NOVA_AI_API_KEY) {
        Write-Fail @"
ListenHost is $ListenHost (non-loopback) but `$env:NOVA_AI_API_KEY is
...
"@
    ...
    # `nova serve` would refuse to bind 0.0.0.0 — failing silently every
    ...
    if (-not $isLoopback) {
        Write-Info "Persisting NOVA_AI_API_KEY to User environment so the scheduled task can read it at logon."
    ...
    $action = New-ScheduledTaskAction `
        -Execute $uvPath `
        -Argument "run nova serve --host $ListenHost --port $ListenPort" `
    ...
    Register-ScheduledTask `
        ...
        -Settings $settings `
        -Principal $principal `
        -Description 'NOVA AI API server (loopback default — see deploy/windows/README.md)' | Out-Null
