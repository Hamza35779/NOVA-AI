...
# NOVA AI's self-hosted PostHog analytics backend.
#
...
#
# Usage:
#   sudo bash posthog-hetzner-prep.sh analytics.nova_ai.ai you@nova_ai.ai
#
# After it finishes:
#   1. Visit https://<DOMAIN>/ and create the admin account.
#   2. Create project "NOVA AI".
#   3. Settings → Project → grab the Project API Key (phc_…).
#   4. Update src/nova_ai/core/config.py AnalyticsConfig defaults:
#        host = "https://<DOMAIN>"
#        key  = "phc_<new>"
...
Examples:
  sudo bash posthog-hetzner-prep.sh analytics.nova_ai.ai team@nova_ai.ai
...
Next steps:
  ...
  3. Create project "NOVA AI".
  4. Settings → Project → Project API Key — copy the phc_… value.
  5. Update src/nova_ai/core/config.py AnalyticsConfig defaults:
        host = "https://$DOMAIN"
  ...
  8. Ship a release of NOVA AI with the new config defaults.
