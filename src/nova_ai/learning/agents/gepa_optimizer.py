...
Uses GEPA's adapter pattern to bridge NOVA AI traces into GEPA's
...
from nova_ai.core.config import GEPAOptimizerConfig
from nova_ai.core.registry import LearningRegistry
from nova_ai.learning._stubs import AgentLearningPolicy
...
class NovaAIGEPAAdapter:
    """Implements GEPA's adapter protocol for NOVA AI agents.
...
class GEPAAgentOptimizer:
    ...
    def optimize(self, trace_store: Any) -> Dict[str, Any]:
        ...
        if not HAS_GEPA:
            return {
                "status": "error",
                "reason": ("gepa not installed (pip install 'nova_ai[learning-gepa]')"),
            }

        agent_name = self.config.agent_filter or "default"
        adapter = NovaAIGEPAAdapter(trace_store, agent_name, self.config)
        adapter.load_traces()
...
__all__ = ["GEPAAgentOptimizer", "NovaAIGEPAAdapter"]
