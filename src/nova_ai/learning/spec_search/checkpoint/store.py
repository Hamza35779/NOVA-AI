...
A thin wrapper over a local git repository at ``<nova_ai_home>/.git``.
...
from nova_ai.learning.spec_search.storage.paths import (
    ConfigurationError,
    _find_source_root,
)
...
class CheckpointStore:
    ...
    root :
        ...
        ``~/.nova_ai/`` (or ``$NOVA_AI_HOME``); for tests it's a
    ...
    def init(self) -> None:
        ...
        Refuses to initialize if ``self.root`` is inside the NOVA AI source
        ...
        if source_root is not None:
            try:
                ...
                raise ConfigurationError(
                    ...
                    f"NOVA AI source tree ({source_root}). Refusing to "
        ...
        if not (self._root / ".git").exists():
            self._git("init", "-q")
            self._git("config", "user.email", "spec-search@nova_ai.local")
            self._git("config", "user.name", "NOVA AI Spec Search")
        ...
        # Allow empty so init succeeds even on a brand-new nova_ai home.
        self._git(
            "commit",
            "--allow-empty",
            "-q",
