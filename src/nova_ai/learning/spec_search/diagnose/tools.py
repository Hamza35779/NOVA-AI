...
``~/.nova_ai/config.toml``, agent prompts, or tool descriptions.
...
def build_diagnostic_tools(
    *,
    ...
    config :
        Dict with ``config_path`` (Path) and ``nova_ai_home`` (Path).
    benchmark_samples :
    ...
    """
    nova_ai_home = config["nova_ai_home"]
    ...
    def _get_agent_prompt(agent_name: str) -> str:
        prompt_path = nova_ai_home / "agents" / agent_name / "system_prompt.md"
        try:
    ...
    def _get_tool_description(tool_name: str) -> str:
        desc_path = nova_ai_home / "tools" / "descriptions.toml"
        try:
    ...
    return [
        ...
        DiagnosticTool(
            ...
            description="Read the current NOVA AI config.toml.",
