    ...
    def _generate_openai(
        ...
        **kwargs: Any,
        ...
        if response_format is not None:
            from nova_ai.engine._stubs import ResponseFormat
    ...
    def _generate_xai(
        ...
        **kwargs: Any,
        ...
        if self._xai_client is None:
            raise EngineConnectionError(
                "xAI client not available — set XAI_API_KEY and install nova_ai[inference-cloud]"
            )
        kwargs.pop("response_format", None)
        create_kwargs: Dict[str, Any] = {
            "model": model,
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if _is_xai_model(model):
            return self._generate_xai(messages, **kw)
