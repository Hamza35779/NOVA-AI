"""Config-driven fluent builder that wires up a NovaSystem."""
...
class SystemBuilder:
    ...
    def engine_instance(
        ...
        ``nova eval --base-url``). ``build()`` health-checks the instance
    ...
    def build(self) -> NovaSystem:
        ...
        if telemetry_enabled and config.telemetry.gpu_metrics:
            ...
            if energy_monitor is None:
                try:
                    from nova_ai.telemetry.gpu_monitor import GpuMonitor
    ...
    def _resolve_engine(self, config: NovaConfig):
        ...
        # ``nova eval --base-url``) and it is down, substituting whatever
    ...
    def _resolve_memory(self, config):
        try:
            import nova_ai.tools.storage  # noqa: F401 -- trigger registration
    ...
    def _resolve_channel(self, config, bus):
        ...
        try:
            import nova_ai.channels  # noqa: F401 -- trigger registration
            from nova_ai.core.registry import ChannelRegistry
            from nova_ai.system._channel_kwargs import build_channel_kwargs
    ...
    def _setup_sandbox(self, config):
        ...
        if not sandbox_enabled:
            return None
        try:
            from nova_ai.sandbox.runner import ContainerRunner
    ...
    @staticmethod
    def _setup_learning_orchestrator(config: NovaConfig):
        if not config.learning.training_enabled:
            return None
        try:
            from nova_ai.core.config import DEFAULT_CONFIG_DIR
            from nova_ai.learning.learning_orchestrator import (
                LearningOrchestrator,
            )
            from nova_ai.learning.training.lora import LoRATrainingConfig
            from nova_ai.traces.store import TraceStore
