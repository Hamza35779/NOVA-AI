"""CLI for the NOVA AI evaluation framework."""
...
BACKENDS = {
    "nova-direct": "Engine-level inference (local or cloud)",
    "nova-agent": "Agent-level inference with tool calling",
...
def _build_backend(
    ...
    - "nova-direct" and "nova-agent" honor them when
    ...
    if backend_name == "nova-agent":
        from nova_ai.evals.backends.nova_agent import NovaAgentBackend

        return NovaAgentBackend(
        ...
        from nova_ai.evals.backends.external import HermesBackend
        ...
        from nova_ai.evals.backends.external import OpenClawBackend
...
def _build_dataset(benchmark: str, subset: str | None = None):
    ...
    if benchmark == "supergpqa":
        from nova_ai.evals.datasets.supergpqa import SuperGPQADataset

        return SuperGPQADataset()
    elif benchmark == "gpqa":
        from nova_ai.evals.datasets.gpqa import GPQADataset

        return GPQADataset()
    elif benchmark == "mmlu-pro":
        from nova_ai.evals.datasets.mmlu_pro import MMLUProDataset

        return MMLUProDataset()
    elif benchmark == "math500":
        from nova_ai.evals.datasets.math500 import MATH500Dataset

        return MATH500Dataset()
    elif benchmark == "natural-reasoning":
        from nova_ai.evals.datasets.natural_reasoning import NaturalReasoningDataset

        return NaturalReasoningDataset()
    elif benchmark == "hle":
        from nova_ai.evals.datasets.hle import HLEDataset

        return HLEDataset()
    elif benchmark == "simpleqa":
        from nova_ai.evals.datasets.simpleqa import SimpleQADataset

        return SimpleQADataset()
    elif benchmark == "wildchat":
        from nova_ai.evals.datasets.wildchat import WildChatDataset

        return WildChatDataset()
    elif benchmark == "ipw":
        from nova_ai.evals.datasets.ipw_mixed import IPWDataset

        return IPWDataset()
    elif benchmark == "gaia":
        from nova_ai.evals.datasets.gaia import GAIADataset

        return GAIADataset()
    elif benchmark == "frames":
        from nova_ai.evals.datasets.frames import FRAMESDataset

        return FRAMESDataset()
    elif benchmark == "swebench":
        from nova_ai.evals.datasets.swebench import SWEBenchDataset

        return SWEBenchDataset()
    elif benchmark == "swefficiency":
        from nova_ai.evals.datasets.swefficiency import SWEfficiencyDataset

        return SWEfficiencyDataset()
    elif benchmark == "terminalbench":
        from nova_ai.evals.datasets.terminalbench import TerminalBenchDataset

        return TerminalBenchDataset()
    elif benchmark == "terminalbench-native":
        from nova_ai.evals.datasets.terminalbench_native import (
            TerminalBenchNativeDataset,
        )

        return TerminalBenchNativeDataset()
    elif benchmark == "terminalbench-v2.1":
        from nova_ai.evals.datasets.terminalbench_v2_1 import (
            TerminalBenchV21Dataset,
        )

        return TerminalBenchV21Dataset()
    elif benchmark == "email_triage":
        from nova_ai.evals.datasets.email_triage import EmailTriageDataset

        return EmailTriageDataset()
    elif benchmark == "morning_brief":
        from nova_ai.evals.datasets.morning_brief import MorningBriefDataset

        return MorningBriefDataset()
    elif benchmark == "research_mining":
        from nova_ai.evals.datasets.research_mining import ResearchMiningDataset

        return ResearchMiningDataset()
    elif benchmark == "knowledge_base":
        from nova_ai.evals.datasets.knowledge_base import KnowledgeBaseDataset

        return KnowledgeBaseDataset()
    elif benchmark == "coding_task":
        from nova_ai.evals.datasets.coding_task import CodingTaskDataset

        return CodingTaskDataset()
    elif benchmark == "loghub":
        from nova_ai.evals.datasets.loghub import LogHubDataset

        return LogHubDataset()
    elif benchmark == "ama-bench":
        from nova_ai.evals.datasets.ama_bench import AMABenchDataset

        return AMABenchDataset()
    elif benchmark == "lifelong-agent":
        from nova_ai.evals.datasets.lifelong_agent import LifelongAgentDataset

        return LifelongAgentDataset(subset=subset or "db_bench")
    elif benchmark == "deepplanning":
        from nova_ai.evals.datasets.deepplanning import DeepPlanningDataset

        return DeepPlanningDataset()
    elif benchmark == "paperarena":
        from nova_ai.evals.datasets.paperarena import PaperArenaDataset

        return PaperArenaDataset()
    elif benchmark == "webchorearena":
        from nova_ai.evals.datasets.webchorearena import WebChoreArenaDataset

        return WebChoreArenaDataset()
    elif benchmark == "workarena":
        from nova_ai.evals.datasets.workarena import WorkArenaDataset

        return WorkArenaDataset()
    elif benchmark == "coding_assistant":
        from nova_ai.evals.datasets.coding_assistant import CodingAssistantDataset

        return CodingAssistantDataset()
    elif benchmark == "security_scanner":
        from nova_ai.evals.datasets.security_scanner import SecurityScannerDataset

        return SecurityScannerDataset()
    elif benchmark == "daily_digest":
        from nova_ai.evals.datasets.daily_digest import DailyDigestDataset

        return DailyDigestDataset()
    elif benchmark == "doc_qa":
        from nova_ai.evals.datasets.doc_qa import DocQADataset

        return DocQADataset()
    elif benchmark == "browser_assistant":
        from nova_ai.evals.datasets.browser_assistant import BrowserAssistantDataset

        return BrowserAssistantDataset()
    elif benchmark == "pinchbench":
        from nova_ai.evals.datasets.pinchbench import PinchBenchDataset

        return PinchBenchDataset(path=subset)
    elif benchmark == "taubench":
        from nova_ai.evals.datasets.taubench import TauBenchDataset

        domains = subset.split(",") if subset else None
        return TauBenchDataset(domains=domains)
    elif benchmark == "livecodebench":
        from nova_ai.evals.datasets.livecodebench import LiveCodeBenchDataset

        return LiveCodeBenchDataset()
    elif benchmark in ("liveresearch", "deepresearch"):
        from nova_ai.evals.datasets.liveresearch import LiveResearchBenchDataset

        return LiveResearchBenchDataset(path=subset)
    elif benchmark == "liveresearchbench":
        from nova_ai.evals.datasets.liveresearchbench import (
            LiveResearchBenchDataset as LRBDataset,
        )

        return LRBDataset()
    elif benchmark == "toolcall15":
        from nova_ai.evals.datasets.toolcall15 import ToolCall15Dataset
...
def _build_scorer(benchmark: str, judge_backend, judge_model: str):
    ...
    if benchmark == "supergpqa":
        from nova_ai.evals.scorers.supergpqa_mcq import SuperGPQAScorer

        return SuperGPQAScorer(judge_backend, judge_model)
    elif benchmark == "gpqa":
        from nova_ai.evals.scorers.gpqa_mcq import GPQAScorer

        return GPQAScorer(judge_backend, judge_model)
    elif benchmark == "mmlu-pro":
        from nova_ai.evals.scorers.mmlu_pro_mcq import MMLUProScorer

        return MMLUProScorer(judge_backend, judge_model)
    elif benchmark == "math500" or benchmark == "natural-reasoning":
        from nova_ai.evals.scorers.reasoning_judge import ReasoningJudgeScorer

        return ReasoningJudgeScorer(judge_backend, judge_model)
    elif benchmark == "hle":
        from nova_ai.evals.scorers.hle_judge import HLEScorer

        return HLEScorer(judge_backend, judge_model)
    elif benchmark == "simpleqa":
        from nova_ai.evals.scorers.simpleqa_judge import SimpleQAScorer

        return SimpleQAScorer(judge_backend, judge_model)
    elif benchmark == "wildchat":
        from nova_ai.evals.scorers.wildchat_judge import WildChatScorer

        return WildChatScorer(judge_backend, judge_model)
    elif benchmark == "ipw":
        from nova_ai.evals.scorers.ipw_mixed import IPWMixedScorer

        return IPWMixedScorer(judge_backend, judge_model)
    elif benchmark == "gaia":
        from nova_ai.evals.scorers.gaia_exact import GAIAScorer

        return GAIAScorer(judge_backend, judge_model)
    elif benchmark == "frames":
        from nova_ai.evals.scorers.frames_judge import FRAMESScorer

        return FRAMESScorer(judge_backend, judge_model)
    elif benchmark == "swebench":
        from nova_ai.evals.scorers.swebench_structural import SWEBenchScorer

        return SWEBenchScorer(judge_backend, judge_model)
    elif benchmark == "swefficiency":
        from nova_ai.evals.scorers.swefficiency_structural import SWEfficiencyScorer

        return SWEfficiencyScorer(judge_backend, judge_model)
    elif benchmark == "terminalbench":
        from nova_ai.evals.scorers.terminalbench_judge import TerminalBenchScorer

        return TerminalBenchScorer(judge_backend, judge_model)
    elif benchmark == "terminalbench-native":
        from nova_ai.evals.scorers.terminalbench_native_structural import (
            TerminalBenchNativeScorer,
        )

        return TerminalBenchNativeScorer(judge_backend, judge_model)
    elif benchmark == "terminalbench-v2.1":
        from nova_ai.evals.scorers.terminalbench_v2_1 import (
            TerminalBenchV21Scorer,
        )

        return TerminalBenchV21Scorer(judge_backend, judge_model)
    elif benchmark == "email_triage":
        from nova_ai.evals.scorers.email_triage import EmailTriageScorer

        return EmailTriageScorer(judge_backend, judge_model)
    elif benchmark == "morning_brief":
        from nova_ai.evals.scorers.morning_brief import MorningBriefScorer

        return MorningBriefScorer(judge_backend, judge_model)
    elif benchmark == "research_mining":
        from nova_ai.evals.scorers.research_mining import ResearchMiningScorer

        return ResearchMiningScorer(judge_backend, judge_model)
    elif benchmark == "knowledge_base":
        from nova_ai.evals.scorers.knowledge_base import KnowledgeBaseScorer
        ...
        from nova_ai.evals.scorers.loghub_scorer import LogHubScorer

        return LogHubScorer(judge_backend, judge_model)
    elif benchmark == "ama-bench":
        from nova_ai.evals.scorers.ama_bench_judge import AMABenchScorer

        return AMABenchScorer(judge_backend, judge_model)
    elif benchmark == "lifelong-agent":
        from nova_ai.evals.scorers.lifelong_agent_scorer import LifelongAgentScorer

        return LifelongAgentScorer(judge_backend, judge_model)
    elif benchmark == "deepplanning":
        from nova_ai.evals.scorers.deepplanning_scorer import DeepPlanningScorer

        return DeepPlanningScorer(judge_backend, judge_model)
    elif benchmark == "paperarena":
        from nova_ai.evals.scorers.paperarena_judge import PaperArenaScorer

        return PaperArenaScorer(judge_backend, judge_model)
    elif benchmark == "webchorearena":
        from nova_ai.evals.scorers.webchorearena_scorer import WebChoreArenaScorer

        return WebChoreArenaScorer(judge_backend, judge_model)
    elif benchmark == "workarena":
        from nova_ai.evals.scorers.workarena_scorer import WorkArenaScorer

        return WorkArenaScorer(judge_backend, judge_model)
    elif benchmark == "coding_assistant":
        from nova_ai.evals.scorers.coding_assistant import CodingAssistantScorer

        return CodingAssistantScorer(judge_backend, judge_model)
    elif benchmark == "security_scanner":
        from nova_ai.evals.scorers.security_scanner import SecurityScannerScorer

        return SecurityScannerScorer(judge_backend, judge_model)
    elif benchmark == "daily_digest":
        from nova_ai.evals.scorers.daily_digest import DailyDigestScorer

        return DailyDigestScorer(judge_backend, judge_model)
    elif benchmark == "doc_qa":
        from nova_ai.evals.scorers.doc_qa import DocQAScorer

        return DocQAScorer(judge_backend, judge_model)
    elif benchmark == "browser_assistant":
        from nova_ai.evals.scorers.browser_assistant import BrowserAssistantScorer

        return BrowserAssistantScorer(judge_backend, judge_model)
    elif benchmark == "pinchbench":
        from nova_ai.evals.scorers.pinchbench import PinchBenchScorer

        return PinchBenchScorer(judge_backend, judge_model)
    elif benchmark == "taubench":
        from nova_ai.evals.scorers.taubench import TauBenchScorer

        return TauBenchScorer(judge_backend, judge_model)
    elif benchmark == "livecodebench":
        from nova_ai.evals.scorers.livecodebench import LiveCodeBenchScorer

        return LiveCodeBenchScorer(judge_backend, judge_model)
    elif benchmark in ("liveresearch", "deepresearch"):
        from nova_ai.evals.scorers.liveresearch import LiveResearchBenchScorer

        return LiveResearchBenchScorer(judge_backend, judge_model)
    elif benchmark == "liveresearchbench":
        from nova_ai.evals.scorers.liveresearchbench import (
            LiveResearchBenchScorer as LRBScorer,
        )

        return LRBScorer(judge_backend, judge_model)
    elif benchmark == "toolcall15":
        from nova_ai.evals.scorers.toolcall15 import ToolCall15Scorer
...
def _build_trackers(config) -> list:
    ...
    if getattr(config, "wandb_project", ""):
        try:
            from nova_ai.evals.trackers.wandb_tracker import WandbTracker
    ...
    if getattr(config, "sheets_spreadsheet_id", ""):
        try:
            from nova_ai.evals.trackers.sheets_tracker import SheetsTracker
...
def _run_terminalbench_native(
    ...
    *,
    ...
    ``base_url`` (from ``--base-url`` / NOVA_BACKEND_BASE_URL) targets an
    already-running OpenAI-compatible endpoint; when unset, the legacy local
    vLLM default (http://localhost:8000/v1) is used.
    """
    from nova_ai.engine.openai_compat_engines import normalize_openai_base_url
    from nova_ai.evals.backends.terminalbench_native import (
...
def _run_single(
    ...
    *,
    ...
    base_url = (
        ...
        or os.environ.get("NOVA_BACKEND_BASE_URL")
    )
    api_key = (
        getattr(config, "api_key", None)
        or _metadata.get("api_key")
        or os.environ.get("NOVA_BACKEND_API_KEY")
    )
...
def _run_agentic(
    ...
    *,
    ...
    from nova_ai.evals.core.agentic_runner import AgenticRunner
    from nova_ai.evals.core.event_recorder import EventRecorder
    from nova_ai.evals.core.export import (
    ...
    try:
        from nova_ai.telemetry.energy_monitor import create_energy_monitor
        from nova_ai.telemetry.session import TelemetrySession
    ...
    try:
        from nova_ai.evals.core.export import export_hf_dataset
...
def _run_from_config(
    ...
    *,
    ...
    from nova_ai.evals.core.config import expand_suite, load_eval_config
...
@click.option(
    ...
    help=(
        ...
        "hermes/openclaw; for nova-direct/nova-agent/terminalbench-native "
        "it bypasses engine discovery and targets this URL directly "
        "(env: NOVA_BACKEND_BASE_URL)."
    ),
)
@click.option(
    "--api-key",
...
@click.option(
    ...
    help="Agent name for nova-agent backend",
)
...
def run(
    ...
    config = RunConfig(
        ...
        base_url=base_url or os.environ.get("NOVA_BACKEND_BASE_URL"),
        api_key=api_key or os.environ.get("NOVA_BACKEND_API_KEY"),
        metadata={
            "base_url": base_url or os.environ.get("NOVA_BACKEND_BASE_URL"),
            "api_key": api_key or os.environ.get("NOVA_BACKEND_API_KEY"),
        },
...
def run_all(
    ...
    for i, bench_name in enumerate(BENCHMARKS, 1):
        ...
        eval_backend = _build_backend("nova-direct", engine_key, "orchestrator", [])
...
def reparse_judge(jsonl_path, out_path, in_place, summary_out):
    ...
    from nova_ai.evals.scorers.liveresearch import rescore_from_metadata
