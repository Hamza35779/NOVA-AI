...
dropped for nova-direct/nova-agent and ignored by terminalbench-native
...
from nova_ai.evals.cli import _build_backend, _run_terminalbench_native
...
class TestBuildBackendForwardsEndpoint:
    @patch("nova_ai.evals.backends.nova_direct.NovaDirectBackend")
    def test_nova_direct_receives_base_url_and_api_key(self, mock_cls):
        _build_backend(
            "nova-direct",
            "vllm",
            "orchestrator",
    ...
    @patch("nova_ai.evals.backends.nova_agent.NovaAgentBackend")
    def test_nova_agent_receives_base_url_and_api_key(self, mock_cls):
        _build_backend(
            "nova-agent",
            "vllm",
            "orchestrator",
...
class TestTerminalBenchNativeApiBase:
    @patch("nova_ai.evals.backends.terminalbench_native.TerminalBenchNativeBackend")
...
class TestRunSingleSuiteModeGating:
    @patch("nova_ai.evals.cli._run_terminalbench_native")
