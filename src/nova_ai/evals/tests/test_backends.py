...
class TestNovaDirectBackend:
    @patch("nova_ai.system.SystemBuilder")
    def test_construction_default(self, mock_builder_cls):
        ...
        backend = NovaDirectBackend()
        assert backend.backend_id == "nova-direct"
    ...
    def test_construction_with_engine_key(self, mock_builder_cls):
        ...
        NovaDirectBackend(engine_key="cloud")
...
class TestNovaAgentBackend:
    ...
    def test_construction(self, mock_builder_cls):
        ...
        backend = NovaAgentBackend(
            engine_key="cloud",
            agent_name="orchestrator",
            tools=["calculator", "think"],
        )
        assert backend.backend_id == "nova-agent"
        mock_builder.engine.assert_called_with("cloud")
    ...
    def test_generate_full(self, mock_builder_cls):
        ...
        backend = NovaAgentBackend(agent_name="orchestrator")
...
class TestNovaDirectBackendBaseUrl:
    """--base-url targeting for the nova-direct backend."""

    @patch("nova_ai.system.SystemBuilder")
    def test_base_url_injects_pinned_openai_compat_engine(self, mock_builder_cls):
        from nova_ai.engine.openai_compat_engines import OpenAICompatEngine
        from nova_ai.evals.backends.nova_direct import NovaDirectBackend
        ...
        with respx.mock:
            ...
            NovaDirectBackend(base_url="http://127.0.0.1:18999/v1", api_key="sk-x")
    ...
    def test_unreachable_base_url_fails_fast_naming_url(self, mock_builder_cls):
        ...
        with respx.mock:
            ...
            with pytest.raises(RuntimeError, match=r"http://127\.0\.0\.1:18998"):
                NovaDirectBackend(base_url="http://127.0.0.1:18998")
    ...
    def test_no_base_url_keeps_engine_key_path(self, mock_builder_cls):
        ...
        NovaDirectBackend(engine_key="vllm")
    ...
    def test_base_url_wins_over_engine_key(self, mock_builder_cls):
        ...
        with respx.mock:
            ...
            NovaDirectBackend(engine_key="vllm", base_url="http://127.0.0.1:18999")
...
class TestNovaAgentBackendBaseUrl:
    """--base-url targeting for the nova-agent backend."""
    ...
    def test_base_url_injects_pinned_openai_compat_engine(self, mock_builder_cls):
        ...
        with respx.mock:
            ...
            NovaAgentBackend(base_url="http://127.0.0.1:18999/v1", api_key="sk-x")
    ...
    def test_unreachable_base_url_fails_fast_naming_url(self, mock_builder_cls):
        ...
        with respx.mock:
            ...
            with pytest.raises(RuntimeError, match=r"http://127\.0\.0\.1:18998"):
                NovaAgentBackend(base_url="http://127.0.0.1:18998")
    ...
    def test_no_base_url_keeps_engine_key_path(self, mock_builder_cls):
        ...
        NovaAgentBackend(engine_key="vllm")
        mock_builder.engine.assert_called_with("vllm")
        mock_builder.engine_instance.assert_not_called()
