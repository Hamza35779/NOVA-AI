"""Nova Agent backend — agent-level inference with tool calling."""
...
from nova_ai.evals.backends._commit_util import nova_ai_commit
from nova_ai.evals.core.backend import InferenceBackend


class NovaAgentBackend(InferenceBackend):
    """Agent-level inference via SystemBuilder + NovaSystem.ask().

    Supports tool calling via the agent harness. Works for both local
    and cloud models.
    """

    backend_id = "nova-agent"
    framework_name = "nova_ai"
    ...
    def __init__(
        ...
        if base_url:
            ...
            from nova_ai.evals.backends._endpoint_util import (
        ...
        # Override the agent's per-run turn budget. NovaConfig.agent.max_turns
    ...
    def framework_commit_value(self) -> str:
        """NOVA AI repo HEAD commit (for telemetry tagging)."""
        from nova_ai.evals.backends._commit_util import nova_ai_commit

        return nova_ai_commit()
    ...
    def generate_full(
        ...
        *,
        ...
        return {
            ...
            "framework": "nova_ai",
            "framework_commit": nova_ai_commit(),
            "error": None,
        }
