...
def load_eval_config(path: str | Path) -> EvalSuiteConfig:
    ...
    for b in benchmarks_raw:
        ...
        backend = b.get("backend", "nova-direct")
    ...
    backend_external_base_url = (
        os.environ.get("NOVA_BACKEND_BASE_URL") or external_raw.get("base_url") or None
    )
    backend_external_api_key = (
        os.environ.get("NOVA_BACKEND_API_KEY") or external_raw.get("api_key") or None
    )
