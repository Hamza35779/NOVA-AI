...
Used by both ``install.sh`` (via ``nova _bootstrap --write-config``)
and ``nova init`` (so there is a single source of truth for the
...
def write_initial_config(
    *,
    ...
    Called by both ``install.sh`` (via ``nova _bootstrap --write-config``)
    and ``nova init`` so the TOML format has one definition.
    """
    ...
    base_toml = (
        f"# NOVA AI configuration\n"
        f"{hardware_line}{gpu_comment}\n"
        f"# Full reference config: nova init --full\n"
        f"\n"
        f"{provenance}\n"
        f"\n"
        f"[engine]\n"
...
# ---------------------------------------------------------------------------
# CLI command — invoked by install.sh, hidden from `nova --help`
# ---------------------------------------------------------------------------
