"""``nova deep-research-setup`` — auto-detect local sources, ingest, and chat.
...
def _instantiate_connector(connector_id: str, config: Dict[str, Any]) -> Any:
    ...
    if connector_id == "apple_notes":
        from nova_ai.connectors.apple_notes import AppleNotesConnector

        return AppleNotesConnector(db_path=config.get("db_path", ""))
    elif connector_id == "imessage":
        from nova_ai.connectors.imessage import IMessageConnector

        return IMessageConnector(db_path=config.get("db_path", ""))
    elif connector_id == "obsidian":
        from nova_ai.connectors.obsidian import ObsidianConnector
        ...
        from nova_ai.connectors.outlook import OutlookConnector

        return OutlookConnector()
    elif connector_id == "slack":
        from nova_ai.connectors.slack_connector import SlackConnector

        return SlackConnector()
    elif connector_id == "notion":
        from nova_ai.connectors.notion import NotionConnector

        return NotionConnector()
    elif connector_id == "granola":
        from nova_ai.connectors.granola import GranolaConnector
...
def ingest_sources(
    ...
    *,
    ...
    state_db:
        ...
        ``~/.nova_ai/sync_state.db`` when empty.
