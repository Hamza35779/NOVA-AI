"""``nova channels`` — manage messaging channels for the agent."""
...
def channels_status() -> None:
    ...
    from nova_ai.channels.imessage_daemon import is_running
    ...
    if is_running():
        ...
        table.add_row(
            "iMessage",
            "[dim]stopped[/dim]",
            "nova channels imessage-start <contact>",
        )
    ...
    if background:
        ...
        proc = subprocess.Popen(
            [
                sys.executable,
                "-m",
                "nova_ai.channels.imessage_daemon",
                "--chat",
                chat_identifier,
            ],
        ...
        from nova_ai.connectors.retriever import (
            TwoStageRetriever,
        )
        from nova_ai.connectors.store import KnowledgeStore
        from nova_ai.engine.ollama import OllamaEngine
        from nova_ai.tools.knowledge_search import (
            KnowledgeSearchTool,
        )
        from nova_ai.tools.knowledge_sql import (
            KnowledgeSQLTool,
        )
...
def imessage_stop() -> None:
    ...
    from nova_ai.channels.imessage_daemon import stop_daemon
