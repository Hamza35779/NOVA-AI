"""``nova operators`` — operator lifecycle management commands."""
...
def status_operators() -> None:
    """Show activation state and health for all discovered operators."""
    console = Console(stderr=True)
    try:
        ...
        table = Table(title="Operator Status")
        table.add_column("ID", style="cyan")
        ...
        for row in rows:
            color = health_colors.get(row["health"], "white")
            fails = str(row["consecutive_failures"])
            if row["consecutive_failures"] > 0:
                fails = f"[red]{fails}[/red]"
            table.add_row(
                row["id"],
                row["status"],
                f"[{color}]{row['health']}[/{color}]",
                _short_ts(row.get("last_run")),
                _short_ts(row.get("next_run")),
                fails,
            )
        console.print(table)

        failing = [
            r for r in rows if r["health"] in ("failing", "stalled") and r["last_error"]
        ]
        if failing:
...
def info(operator_id: str) -> None:
    ...
    try:
        ...
        if manifest.required_capabilities:
            console.print(
                f"  Required capabilities: {', '.join(manifest.required_capabilities)}"
            )
        if manifest.rate_limit_rpm > 0:
            console.print(f"  Rate limit: {manifest.rate_limit_rpm} runs/min")
        if manifest.max_consecutive_failures > 0:
            console.print(
                f"  Auto-pause after: {manifest.max_consecutive_failures} "
...
def _short_ts(value: str | None) -> str:
    """Compact an ISO timestamp for table display (date + HH:MM)."""
    if not value:
        return "—"
...
def _build_system_with_operators():
    """Build a NovaSystem with operators wired up."""
