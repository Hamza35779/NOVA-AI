"""``nova optimize`` — LLM-driven configuration optimization CLI."""
...
def optimize_run(
    ...
    """Run an optimization loop over NOVA AI configuration."""
    ...
    if config_path is not None:
        try:
            from nova_ai.learning.optimize.config import load_optimize_config
        except ImportError:
    ...
    if data is not None:
        try:
            from nova_ai.learning.optimize.config import load_benchmark_specs
    ...
    try:
        ...
        from nova_ai.learning.optimize.config import load_objectives
...
def skills(policy: str, min_traces: int, dry_run: bool) -> None:
    ...
    from nova_ai.learning.agents.skill_optimizer import SkillOptimizer
