...
def install(query: str, with_scripts: bool, force: bool, url: str):
    ...
    Example: ``nova skill install hermes:apple-notes``
    """
    console = Console()
...
def remove(skill_name: str, yes: bool):
    ...
    Searches ``~/.nova_ai/skills/`` and ``./skills`` for a directory whose
...
def search(query: str, source: str):
    ...
    console.print(
        ...
        f"Install with: nova skill install <source>:<name>[/dim]"
    )
...
def discover(min_frequency: int, min_outcome: float, dry_run: bool) -> None:
    ...
    discovered skill manifests under ~/.nova_ai/skills/discovered/."""
    console = Console()
    ...
    if dry_run:
        ...
        tmp = Path(tempfile.mkdtemp(prefix="nova_ai-discover-dryrun-"))
        try:
...
def show_overlay(skill_name: str) -> None:
    ...
    console = Console()
    from nova_ai.skills.overlay import SkillOverlayLoader
...
@skill.command("import-local")
@click.argument("local_path", type=click.Path(exists=True))
@click.option(
    "--source-name",
    default="local",
    help="Source namespace name (e.g. 'openclaw' or 'local').",
)
@click.option(
    "--with-scripts", is_flag=True, default=False, help="Import scripts directory."
)
@click.option(
    "--force", is_flag=True, default=False, help="Overwrite existing install."
)
def import_local(
    local_path: str, source_name: str, with_scripts: bool, force: bool
) -> None:
    """Import skills from a local directory or SKILL.md file.

    Examples:
        nova skill import-local "d:/My Softwares/Friday AI/openclaw-main/skills" --source-name openclaw
        nova skill import-local "./my_skill/SKILL.md"
    """
    console = Console()
    target_path = Path(local_path)
    skill_files: List[Path] = []

    if target_path.is_file() and target_path.name.lower() == "skill.md":
        skill_files.append(target_path)
    elif target_path.is_dir():
        # Look for direct SKILL.md or nested SKILL.md files
        skill_files = list(target_path.rglob("SKILL.md")) + list(
            target_path.rglob("skill.md")
        )
        # Deduplicate
        skill_files = list({p.resolve(): p for p in skill_files}.values())

    if not skill_files:
        console.print(f"[red]No SKILL.md files found in '{local_path}'.[/red]")
        raise SystemExit(1)

    from nova_ai.skills.importer import SkillImporter
    from nova_ai.skills.parser import SkillParser
    from nova_ai.skills.sources.base import ResolvedSkill
    from nova_ai.skills.tool_translator import ToolTranslator

    parser = SkillParser()
    importer = SkillImporter(parser=parser, tool_translator=ToolTranslator())

    console.print(
        f"[bold cyan]Found {len(skill_files)} skill(s) to import...[/bold cyan]"
    )
    success_count = 0

    for s_file in skill_files:
        skill_dir = s_file.parent
        skill_name = skill_dir.name

        # Parse frontmatter to get description if possible
        desc = ""
        try:
            parsed = parser.parse_file(s_file)
        ...
        res = importer.import_skill(resolved, with_scripts=with_scripts, force=force)
        if res.success:
            status_tag = (
                "[yellow](skipped - already exists)[/yellow]"
                if res.skipped
                else "[green]✓ Installed[/green]"
            )
            console.print(f"  {status_tag} {source_name}:{skill_name}")
            if not res.skipped:
                success_count += 1
        else:
            console.print(
                f"  [red]✗ Failed[/red] {source_name}:{skill_name}: {res.warnings}"
            )

    console.print(
        f"\n[bold green]Import complete:[/bold green] {success_count} newly imported."
    )
