"""``nova quickstart`` — guided 5-step setup for new users."""
...
def _check_engine_health(engine_key: str) -> bool:
    ...
    try:
        import nova_ai.engine  # noqa: F401 — trigger registration
...
def quickstart(force: bool) -> None:
    ...
    if not _check_engine_health(engine_key):
        ...
        if fallbacks:
            ...
            console.print("  Run [bold]nova doctor[/bold] for detailed diagnostics.")
            raise SystemExit(1)
    else:
    ...
    console.print()
    console.print(
        '[bold green]Setup complete![/bold green] Try: [bold]nova ask "Hello"[/bold]'
    )
