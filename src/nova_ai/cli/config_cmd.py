"""``nova config`` — configuration inspection commands."""
...
def _get_config_path(path: str | None) -> Path:
    ...
    if path:
        return Path(path)
    return Path(os.environ.get("NOVA_AI_CONFIG", DEFAULT_CONFIG_PATH))


def _show_hardware_info(console: Console, show_recommendations: bool = True) -> None:
    """Display detected hardware information."""
    from nova_ai.core.config import detect_hardware, recommend_engine
...
@config.command("path")
def show_path() -> None:
    """Print the resolved NOVA AI directories (home, config, cache).

    All NOVA AI state lives under a single root, resolved in priority
    order: ``$NOVA_AI_HOME`` > ``$XDG_DATA_HOME/nova_ai`` >
    ``~/.nova_ai``. Use this to confirm where your data is stored after
    setting an override.
    """
    from nova_ai.core.paths import get_cache_dir, get_config_dir, get_config_path

    console = Console(stderr=True)
    home = get_config_dir()
    override = (
        "NOVA_AI_HOME"
        if os.environ.get("NOVA_AI_HOME")
        else "XDG_DATA_HOME"
        if os.environ.get("XDG_DATA_HOME")
        else "default (~/.nova_ai)"
    )
...
def set_config(key: str, value: str) -> None:
    """Set a configuration value (e.g. nova config set engine.ollama.host URL)."""
    import tomlkit

    from nova_ai.core.config import DEFAULT_CONFIG_DIR, validate_config_key
    ...
    config_path = Path(
        os.environ.get("NOVA_AI_CONFIG", DEFAULT_CONFIG_DIR / "config.toml")
    )
    if config_path.exists():
