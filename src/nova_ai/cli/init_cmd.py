"""``nova init`` — detect hardware, generate config, write to disk."""
...
from nova_ai.cli._bootstrap import detect_cloud_keys
from nova_ai.cli.model import find_model_spec, hf_download, ollama_pull
from nova_ai.cli.scan_cmd import PrivacyScanner
...
def _quick_privacy_check(console: Console) -> None:
    ...
    console.print()
    console.print("  Run [cyan]nova scan[/cyan] for a full environment audit.")
...
@click.option(
    ...
    default=False,
    hidden=True,
    help="Run init non-interactively; called by the bare-nova first-run guard.",
)
@click.pass_context
def init(
    ctx: click.Context,
    ...
    from_bare_nova: bool = False,
) -> None:
    """Detect hardware and generate ~/.nova_ai/config.toml."""
    ...
    if preset:
        examples_dir = (
            Path(__file__).resolve().parents[2] / "configs" / "nova_ai" / "examples"
        )
        # Also check installed package location
        if not examples_dir.exists():
            examples_dir = (
                Path(__file__).resolve().parents[3] / "configs" / "nova_ai" / "examples"
            )
        ...
        console.print(
            ...
            "[bold]nova doctor[/bold] to verify."
        )
        return
    ...
    if engine is None and config is None:
        ...
        # Bare-nova cold path: use the recommended engine non-interactively.
        if from_bare_nova:
            engine = recommended
        else:
            console.print()
...
"""
        ...
        console.print(
            ...
            "Run [bold]nova connect gdrive[/bold] to connect "
            "Google services, then [bold]nova digest --fresh[/bold]."
        )
    ...
    if not model:
        ...
        if not no_download and not from_bare_nova and spec:
            ...
            if click.confirm(prompt, default=True):
                ...
                console.print(
                    ...
                    f"    [bold]nova model pull {model}[/bold]"
                )
