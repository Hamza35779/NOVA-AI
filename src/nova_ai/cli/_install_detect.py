...
- **PyPI** (``pip install nova_ai``). The package lives somewhere
  inside ``site-packages``. Upgrade with ``pip install --upgrade nova_ai``.
- **uv tool** (``uv tool install nova_ai``). Lives in a uv-managed
  isolated venv under ``~/.local/share/uv/tools/``. Upgrade with
  ``uv tool upgrade nova_ai``.
...
def detect_install() -> InstallInfo:
    ...
    try:
        ...
        return InstallInfo(
            kind="unknown",
            upgrade_command="pip install --upgrade nova_ai",
        )
    ...
    if "uv" in parts and "tools" in parts:
        return InstallInfo(
            kind="uv-tool",
            upgrade_command="uv tool upgrade nova_ai",
        )
