"""Detect how NOVA AI was installed so we can show the right upgrade
command (and run the right upgrade command for ``nova self-update``).

Three install paths are supported today:

- **PyPI** (``pip install nova-ai-pro``). The package lives somewhere
  inside ``site-packages``. Upgrade with ``pip install --upgrade nova-ai-pro``.
- **uv tool** (``uv tool install nova-ai-pro``). Lives in a uv-managed
  isolated venv under ``~/.local/share/uv/tools/``. Upgrade with
  ``uv tool upgrade nova-ai-pro``.
...
We detect by inspecting ``nova_ai.__file__``. If we can't tell with
...
def detect_install() -> InstallInfo:
    ...
    Cheap: just walks the parent directories of ``nova_ai.__file__``
    once and checks for marker directories. No subprocess calls.
    """
    try:
        import nova_ai

        pkg_file = Path(nova_ai.__file__).resolve()
    except Exception:
        return InstallInfo(
            kind="unknown",
            upgrade_command="pip install --upgrade nova-ai-pro",
        )
