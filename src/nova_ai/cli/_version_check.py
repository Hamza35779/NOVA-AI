"""Check for newer NOVA AI releases on PyPI."""
...
def _config_path() -> Path:
    """Resolve the config path, honoring ``NOVA_AI_CONFIG`` like core.config."""
    override = os.environ.get("NOVA_AI_CONFIG")
    if override:
...
# - ``NOVA_AI_NO_UPDATE_CHECK=1`` — project-specific
# - ``CI=true`` — set by every major CI provider, suppresses by default
_OPT_OUT_ENV_VARS = ("NOVA_AI_NO_UPDATE_CHECK",)
...
def _check_disabled() -> bool:
    ...
    # ``NOVA_AI_NO_UPDATE_CHECK=0`` if they want the nudge anyway.
...
def check_for_updates(command_name: str) -> None:
    ...
    Honors ``NOVA_AI_NO_UPDATE_CHECK=1`` and ``CI=true`` — any
...
def _do_check() -> None:
    ...
    try:
        if Version(latest) > Version(current):
            from nova_ai.cli._install_detect import detect_install

            cmd = detect_install().upgrade_command
            sys.stderr.write(
                f"\033[33mA new version of NOVA AI is available "
                f"(v{current} → v{latest})\n"
                f"Update: {cmd}\n"
                f"Or run: nova self-update\033[0m\n\n"
            )
