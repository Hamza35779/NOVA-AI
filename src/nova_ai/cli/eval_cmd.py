"""``nova eval`` — evaluation framework CLI commands."""
...
@click.option(
    ...
    default="nova-direct",
    type=click.Choice(
        ["nova-direct", "nova-agent", "hermes", "openclaw", "terminalbench-native"]
    ),
    help=(
        "Inference backend. For hermes/openclaw, also pass --base-url and "
        "--api-key (or set NOVA_BACKEND_BASE_URL/NOVA_BACKEND_API_KEY)."
    ),
)
@click.option(
    "--base-url",
...
def eval_run(
    ...
    if config_path is not None:
        ...
        try:
            from nova_ai.evals.cli import _run_single
        except ImportError:
    ...
    config = RunConfig(
        ...
        # NOVA_BACKEND_BASE_URL/NOVA_BACKEND_API_KEY.
