"""``nova scan`` — audit your environment for privacy and security risks."""
...
class PrivacyScanner:
    ...
    def check_icloud_sync(self) -> ScanResult:
        """Check whether ~/.nova_ai is inside iCloud Drive sync scope."""
        try:
            ...
            if str(config_path).startswith(str(icloud_path)):
                return ScanResult(
                    name="iCloud Sync",
                    status="warn",
                    message="~/.nova_ai may be synced to iCloud.",
                    platform="darwin",
                )
            ...
            return ScanResult(
                name="iCloud Sync",
                status="ok",
                message="~/.nova_ai is not inside iCloud Drive.",
                platform="darwin",
            )
        except Exception:
            return ScanResult(
...
def _render_results(results: List[ScanResult]) -> None:
    ...
    console = Console()
    console.print()
    console.print("[bold]NOVA AI Security Scan[/bold]")
    console.print()
    ...
    if fail_count:
        console.print(
            ...
            "before storing sensitive data with NOVA AI."
        )
        console.print()
    elif warn_count:
        console.print(
