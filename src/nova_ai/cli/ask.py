"""``nova ask`` — send a query to the assistant."""
...
def _run_research(
    *,
    ...
    from nova_ai.agents.research_loop import DEFAULT_PLANNER_MODEL, ResearchAgent
    from nova_ai.connectors.embeddings import OllamaEmbedder
    from nova_ai.connectors.hybrid_search import HybridSearch
    ...
    # (so ``nova ask --research "..." > out.md`` still gives a clean
...
def _run_agent(
    ...
    import nova_ai.agents  # noqa: F401
    from nova_ai.agents._stubs import AgentContext
    ...
    from nova_ai.mcp.loader import load_mcp_tools_from_config
    ...
    if config.agent.context_from_memory:
        try:
            from nova_ai.tools.storage.context import ContextConfig, inject_context
...
@click.option(
    ...
    default=None,
    help=(
        "Named persona dir under ~/.nova_ai/personas/<name>/ "
...
def ask(
    ...
    if capture_screen:
        try:
            from nova_ai.cli._screen import capture_screen_to_temp
    ...
    # the most common command (`nova ask "..."`).
    ...
    if resolved is None:
        console.print(
            ...
            "  [cyan]nova config set engine.ollama.host http://<remote-ip>:11434[/cyan]\n"
    ...
    from nova_ai.security import setup_security
    ...
    if not no_context and config.agent.context_from_memory:
        try:
            from nova_ai.tools.storage.context import (
                ContextConfig,
                inject_context,
            )
