"""``nova host`` — download and serve a model locally with auto backend setup."""
...
def host(
    ...
    \b
      nova host mlx-community/Qwen2.5-7B-4bit --backend mlx
      nova host Qwen/Qwen3-8B --backend vllm
      nova host qwen3:8b --backend ollama
      nova host meta-llama/Llama-3-8B -b sglang
    """
    console = Console()
    ...
    if backend != "ollama":
        ...
        console.print(
            "[dim]NOVA AI will auto-discover it. Press Ctrl+C to stop.[/dim]\n"
        )
