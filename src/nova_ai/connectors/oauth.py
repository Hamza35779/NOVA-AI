...
class OAuthProvider:
    ...
    # Filenames in ~/.nova_ai/connectors/ to save tokens to
...
def get_client_credentials(
    ...
    Checks credential files in ``~/.nova_ai/connectors/`` and falls
    back to environment variables ``NOVA_AI_{NAME}_CLIENT_ID`` and
    ``NOVA_AI_{NAME}_CLIENT_SECRET``.
    """
    ...
    prefix = f"NOVA_AI_{provider.name.upper()}"
        ...
        def do_GET(self) -> None:  # noqa: N802 — required override name
            ...
            if "code" in params:
                ...
                self.wfile.write(
                    ...
                    b"<p>You can close this tab and return to NOVA AI.</p>"
                    b"</body></html>"
                )
