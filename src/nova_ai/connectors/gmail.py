...
Uses OAuth 2.0 tokens stored locally (see :mod:`nova_ai.connectors.oauth`).
...
from nova_ai.connectors.google_auth import (
    GoogleAuthError,
)
from nova_ai.connectors.google_auth import (
...
class GmailConnector(BaseConnector):
    ...
    def is_connected(self) -> bool:
        ...
        token), which made `nova connect gmail` short-circuit with
    ...
    def _refresh_token(self) -> str:
        ...
        if not new:
            raise RuntimeError(
                "Gmail token refresh failed — re-run `nova connect gmail`"
            )
        return new
