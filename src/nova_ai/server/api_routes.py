...
async def list_agents(request: Request):
    ...
    registered = []
    try:
        import nova_ai.agents  # noqa: F401 — side-effect registration
    ...
    running = []
    try:
        from nova_ai.tools.agent_tools import _SPAWNED_AGENTS
...
async def create_agent(req: AgentCreateRequest, request: Request):
    """Spawn a new agent."""
    try:
        from nova_ai.tools.agent_tools import AgentSpawnTool
...
async def kill_agent(agent_id: str, request: Request):
    ...
    try:
        from nova_ai.tools.agent_tools import AgentKillTool
...
async def message_agent(agent_id: str, req: AgentMessageRequest, request: Request):
    ...
    try:
        from nova_ai.tools.agent_tools import AgentSendTool
...
def _get_memory_backend(request: Request):
    ...
    cannot be built because the mandatory ``nova_ai_rust`` extension is not
    ...
    if backend is None:
        from nova_ai.tools.storage._stubs import MemoryBackendUnavailable

        try:
            from nova_ai.tools.storage.sqlite import SQLiteMemory
...
async def memory_config(request: Request):
    ...
    ``backend_type: sqlite``) when the native ``nova_ai_rust`` extension is
...
async def memory_index(req: MemoryIndexRequest, request: Request):
    ...
    try:
        ...
        from nova_ai.security.file_policy import is_sensitive_file
        ...
        # Sandbox: when workspace roots are configured via NOVA_AI_WORKSPACE
        ...
        workspace = os.environ.get("NOVA_AI_WORKSPACE", "").strip()
        if workspace:
            roots = [
...
async def list_sessions(request: Request, limit: int = 20):
    ...
    try:
        from nova_ai.sessions.store import SessionStore
...
def _record_ws_trace(
    ...
    *,
    ...
    if trace_store is None or not result:
        return
    from nova_ai.traces.collector import record_response_trace
...
async def websocket_chat_stream(websocket: WebSocket):
    ...
    """
    from nova_ai.server.auth_middleware import websocket_authorized
...
async def learning_stats(request: Request):
    ...
    # Skill discovery
    try:
        from nova_ai.learning.agents.skill_discovery import SkillDiscovery
...
def include_all_routes(app) -> None:
    ...
    from nova_ai.server.approval_routes import (
    ...
    try:
        if hasattr(app.state, "agent_manager") and app.state.agent_manager:
            from nova_ai.server.agent_manager_routes import (  # noqa: PLC0415
    ...
    try:
        ...
        from nova_ai.server.ws_bridge import create_ws_router
