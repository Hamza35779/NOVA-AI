...
def _ensure_identity_prompt(messages: list[Message], app_config) -> list[Message]:
    """Prepend NOVA AI's identity system prompt when the client omits one.
    ...
    try:
        ...
        logging.getLogger("nova_ai.server").debug(
        ...
        try:
            ...
            if query_text:
                ...
                if len(enriched) > len(messages):
                    from nova_ai.server.models import ChatMessage
...
def _handle_agent(
    ...
    *,
    ...
    if audio_path:
        ...
        from nova_ai.server.models import AudioMeta
    ...
    *,
    ...
    async def generate():
        ...
        try:
            ...
            logging.getLogger("nova_ai.server").error(
                "Tool stream error: %s",
                exc,
                exc_info=True,
            )
...
async def _handle_stream(
    ...
    *,
    ...
    from nova_ai.server.cloud_router import (
        is_cloud_model,
        stream_cloud,
        stream_local,
    )
...
async def list_models(request: Request) -> ModelListResponse:
    ...
    """
    from nova_ai.server.cloud_router import is_cloud_model, list_local_models
...
async def reload_cloud_engine(request: Request):
    ...
    # Re-read ~/.nova_ai/cloud-keys.env and update the running process env.
    ...
    try:
        from nova_ai.engine.cloud import CloudEngine
        from nova_ai.engine.multi import MultiEngine
