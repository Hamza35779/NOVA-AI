"""Single point of contact between Python and the Rust ``nova_ai_rust`` module.

Every Python module that wants to delegate to Rust should import helpers from
here rather than importing ``nova_ai_rust`` directly.  The Rust backend is
...
def get_rust_module() -> _types.ModuleType:
    """Return the ``nova_ai_rust`` module.
    ...
    """
    import nova_ai_rust  # type: ignore[import-untyped]

    return nova_ai_rust


def _detect_rust() -> bool:
    """Return ``True`` if the compiled ``nova_ai_rust`` extension is importable.
