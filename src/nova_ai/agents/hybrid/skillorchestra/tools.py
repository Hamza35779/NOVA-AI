...
both forced by the NOVA AI environment and documented inline:
...
* in-tool correctness check — the original ``answer`` tool LLM-judges the
  prediction against the gold answer inside ``call_tool``. NOVA AI
...
def run_answer(
    ...
    *,
    ...
    in-tool LLM correctness check is dropped — NOVA AI scores
    downstream.
    """
...
def run_search(
    ...
    *,
    ...
    NOVA AI substitution for the missing FAISS wiki index).
    """
    prompt = (
        context_str.strip()
        + "\n\n"
