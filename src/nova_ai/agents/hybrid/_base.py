...
Why not just use NOVA AI's :class:`InferenceEngine` for both? Two reasons:
...
from nova_ai.agents.hybrid._openai_retry import patch_openai_globally
...
def tavily_search_context(
    ...
    *,
    ...
    """Run NOVA AI WebSearchTool and return accounting-friendly metadata."""
...
class LocalCloudAgent(BaseAgent):
    ...
    def _call_openrouter(
        ...
        *,
        ...
        pass the NOVA AI-engine-style ``"openrouter/<provider>/<model>"``
        prefix (see ``src/nova_ai/engine/cloud.py``) — we strip it here.
