...
harness — each is registered as a standard NOVA AI agent so the rest
of the platform (SDK, CLI, distillation, evals) can use them like any
other agent. Results live under ``$NOVA_AI_HYBRID_EXPERIMENTS_DIR``
(defaults to ``~/.nova_ai/experiments/hybrid/``).
...
Items in parentheses are what the *paper* trains. These NOVA AI ports
...
`src/nova_ai/evals/scorers/swebench_harness.py` (next to the existing
structural scorer).
...
# 3. Run a smoke cell
.venv/bin/python -m nova_ai.agents.hybrid.runner \
    --cell minions-gaia-qwen27b-opus-3
```

Outputs land in
`$NOVA_AI_HYBRID_EXPERIMENTS_DIR/runs/<cell>/{results.jsonl,summary.json,config.json,logs/}`
(defaults to `~/.nova_ai-hybrid/experiments/`). The schema matches the
...
```bash
src/nova_ai/agents/hybrid/scripts/new_experiment.sh \
...
(`~/.nova_ai/experiments/hybrid/docs/results.md`) at full N —
...
these NOVA AI cells should reproduce the harness numbers within
