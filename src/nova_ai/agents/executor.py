...
from nova_ai.agents.errors import (
    AgentTickError,
    EscalateError,
    FatalError,
    classify_error,
...
class AgentExecutor:
    ...
    Constructor receives a NovaSystem reference for access to engine,
    ...
    def set_system(self, system: Any) -> None:
        """Deferred system injection — called after NovaSystem is constructed."""
        self._system = system
    ...
    def _invoke_agent(self, agent: dict) -> AgentResult:
        ...
        from nova_ai.agents import AgentRegistry
        ...
        # Resolve engine + model from NovaSystem
        engine = self._system.engine if self._system else None
        if engine is None:
            raise FatalError("No engine available in NovaSystem")
        model = (
            config.get("model")
        ...
        # agent's own ToolExecutor. Set by CLI paths like `nova agents ask`
        ...
        if self._system is not None:
            ...
            # agents, mirroring the one-shot `nova ask` path so they no
