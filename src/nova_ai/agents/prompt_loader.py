"""Load system prompt and few-shot overrides from $NOVA_AI_HOME.
...
- System prompts: ``$NOVA_AI_HOME/agents/{name}/system_prompt.md``
- Few-shot exemplars: ``$NOVA_AI_HOME/agents/{name}/few_shot.json``
...
def _nova_ai_home() -> Path:
    """Resolve the NOVA AI root, honoring NOVA_AI_HOME / XDG_DATA_HOME."""
...
def load_system_prompt_override(agent_name: str) -> str | None:
    ...
    Looks for ``$NOVA_AI_HOME/agents/<agent_name>/system_prompt.md``.
    ``NOVA_AI_HOME`` defaults to ``~/.nova_ai`` when unset.
    """
    home = _nova_ai_home()
...
def load_few_shot_exemplars(
    ...
    Looks for ``$NOVA_AI_HOME/agents/<agent_name>/few_shot.json``.
