...
SCHEDULER_TASK_PAUSED = "scheduler_task_paused"
...
class TaskScheduler:
    ...
    system:
        Optional ``NovaSystem`` instance for executing prompts.
    poll_interval:
    ...
    def get_run_logs(self, task_id: str, limit: int = 10) -> List[Dict[str, Any]]:
        """Return recent run logs for *task_id* (newest first)."""
        with self._lock:
            return self._store.get_run_logs(task_id, limit=limit)
    ...
    def _execute_task(self, task: ScheduledTask) -> None:
        ...
        # Rate limiting (metadata: rate_limit_rpm) — skip and reschedule if the
        # task ran more recently than its minimum interval allows.
        if self._rate_limited(task):
            return
    ...
    def _rate_limited(self, task: ScheduledTask) -> bool:
        """Check the task's ``rate_limit_rpm`` metadata against its last run.

        Returns True (and pushes ``next_run`` past the minimum interval) when
        the task ran more recently than its rate limit allows. The last-run
        timestamp is read from the persisted task record so the limit holds
        across restarts.
        """
        meta = task.metadata or {}
        try:
            rpm = int(meta.get("rate_limit_rpm", 0) or 0)
        except (TypeError, ValueError):
            rpm = 0
        if rpm <= 0 or not task.last_run:
            return False
        ...
        try:
            ...
            # Unparseable or naive timestamp — don't block execution on it
            return False
        ...
        with self._lock:
            d = self._store.get_task(task.id)
            if d is not None and d.get("status") == "active":
                d["next_run"] = (last_dt + timedelta(seconds=min_interval)).isoformat()
                self._store.update_task(d)
        logger.info(
            "Rate limit: skipping task %s (%.0fs since last run, min interval %.0fs)",
            task.id,
            max(elapsed, 0.0),
            min_interval,
        )
    ...
    def _check_circuit_breaker(self, task: ScheduledTask) -> None:
        """Auto-pause a task after ``max_consecutive_failures`` failed runs.

        Failure history comes from the persisted run logs, so the breaker
        survives restarts. Paused tasks can be resumed with
        :meth:`resume_task` / ``nova scheduler resume``.
        """
        meta = task.metadata or {}
        try:
            max_failures = int(meta.get("max_consecutive_failures", 0) or 0)
        except (TypeError, ValueError):
            max_failures = 0
        if max_failures <= 0:
            return
        ...
        with self._lock:
            recent = self._store.get_run_logs(task.id, limit=max_failures)
        if len(recent) < max_failures or any(r.get("success") for r in recent):
            return

        with self._lock:
            d = self._store.get_task(task.id)
            if d is None or d.get("status") != "active":
                return
            d["status"] = "paused"
            self._store.update_task(d)

        error_text = recent[0].get("error", "") if recent else ""
        logger.warning(
            "Circuit breaker: task %s paused after %d consecutive failures "
            "(last error: %s)",
            task.id,
            max_failures,
