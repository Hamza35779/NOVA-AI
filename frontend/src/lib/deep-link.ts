/**
 * deep-link.ts — Handler for nova_ai:// deep links in the Tauri desktop app.
 *
 ...
 *   nova_ai://research/{session_id}   → open a research session
 *   nova_ai://connector/{connector_id} → open a connector settings panel
 */
...
/**
 * Parse an `nova_ai://` deep link URL into a structured target.
 *
 * @param url - The raw deep link URL string (e.g. `nova_ai://research/abc123`).
 * @returns A {@link DeepLinkTarget} if the URL is valid, or `null` if it
 *   cannot be parsed or does not use the `nova_ai:` scheme.
 *
 * @example
 * parseDeepLink("nova_ai://research/abc123");
 * // → { type: "research", id: "abc123" }
 *
 * parseDeepLink("nova_ai://connector/gmail");
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    if (parsed.protocol !== "nova_ai:") return null;

    // The URL constructor treats "nova_ai://research/abc123" such that
