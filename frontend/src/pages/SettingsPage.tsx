...
export function SettingsPage() {
  ...
  const [memoryEnabled, setMemoryEnabled] = useState(() => {
    try { return localStorage.getItem('nova_ai-memory-enabled') !== 'false'; } catch { return true; }
  });
  const [memoryBackend, setMemoryBackend] = useState(() => {
    try { return localStorage.getItem('nova_ai-memory-backend') || 'sqlite'; } catch { return 'sqlite'; }
  });
  const [memoryTopK, setMemoryTopK] = useState(() => {
    try { return parseInt(localStorage.getItem('nova_ai-memory-top-k') || '5'); } catch { return 5; }
  });
  const [memoryMinScore, setMemoryMinScore] = useState(() => {
    try { return parseFloat(localStorage.getItem('nova_ai-memory-min-score') || '0.1'); } catch { return 0.1; }
  });
  const [memoryMaxTokens, setMemoryMaxTokens] = useState(() => {
    try { return parseInt(localStorage.getItem('nova_ai-memory-max-tokens') || '2048'); } catch { return 2048; }
  });
  ...
  const handleExport = () => {
    const data = localStorage.getItem('nova_ai-conversations') || '{}';
    const blob = new Blob([data], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `nova_ai-export-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
    ...
    input.onchange = (e) => {
      ...
      reader.onload = (ev) => {
        try {
          ...
          if (data.version === 1) {
            localStorage.setItem('nova_ai-conversations', JSON.stringify(data));
  ...
  const handleClear = () => {
    ...
    localStorage.removeItem('nova_ai-conversations');
    useAppStore.getState().loadConversations();
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex flex-col gap-4">
          ...
          <Section title="Appearance">
            <SettingRow label="Theme" description="Choose how NOVA AI looks">
          ...
          <Section title="Models">
            ...
            <SettingRow label="Cloud providers" description="Green dot means API key is configured">
              <div className="flex flex-wrap gap-3">
                <CloudProviderStatus label="OpenAI" storageKey="nova_ai-openai-key" />
                <CloudProviderStatus label="Anthropic" storageKey="nova_ai-anthropic-key" />
                <CloudProviderStatus label="Google" storageKey="nova_ai-gemini-key" />
                <CloudProviderStatus label="OpenRouter" storageKey="nova_ai-openrouter-key" />
              </div>
            </SettingRow>
          </Section>
          ...
          <Section title="API Keys">
            <SettingRow label="OpenAI" description="GPT-4, GPT-3.5, etc.">
              <ApiKeyInput storageKey="nova_ai-openai-key" placeholder="sk-..." />
            </SettingRow>
            <SettingRow label="Anthropic" description="Claude models">
              <ApiKeyInput storageKey="nova_ai-anthropic-key" placeholder="sk-ant-..." />
            </SettingRow>
            <SettingRow label="Google" description="Gemini models">
              <ApiKeyInput storageKey="nova_ai-gemini-key" placeholder="AI..." />
            </SettingRow>
            <SettingRow label="OpenRouter" description="Multi-provider routing">
              <ApiKeyInput storageKey="nova_ai-openrouter-key" placeholder="sk-or-..." />
            </SettingRow>
          </Section>
          ...
          <Section title="Tools">
            <SettingRow label="Web Search" description="SerpAPI or Tavily key for web search tool">
              <ApiKeyInput storageKey="nova_ai-search-key" placeholder="API key..." />
            </SettingRow>
          </Section>
          ...
          <Section title="Speech">
            ...
            {!speechBackendAvailable && speechBackendAvailable !== null && (
              <div className="text-xs mt-2 px-1" style={{ color: 'var(--color-text-tertiary)' }}>
                ...
                See the <a href="https://hamza35779.github.io/NOVA-AI/user-guide/tools/" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--color-accent)' }}>documentation</a> for details.
              </div>
            )}
          </Section>
          ...
          <Section title="About">
            <div className="text-sm" style={{ color: 'var(--color-text-secondary)' }}>
              <p className="mb-2">
                <span className="font-semibold" style={{ color: 'var(--color-text)' }}>NOVA AI</span> — Programming abstractions for on-device AI.
              </p>
              <div className="flex gap-3 mt-3 text-xs">
                <a
                  href="https://github.com/Hamza35779/NOVA-AI"
                  target="_blank"
                ...
                <a
                  href="https://hamza35779.github.io/NOVA-AI/"
                  target="_blank"
                  rel="noopener noreferrer"
                  style={{ color: 'var(--color-accent)' }}
                >
