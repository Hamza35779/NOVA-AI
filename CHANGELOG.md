...
All notable changes to NOVA AI are documented in this file.
...
*Capability policy enforcement.* `OperatorManager.activate()` now checks
the operator manifest's `required_capabilities` against the system's
RBAC `CapabilityPolicy` before creating the scheduler task; activation
fails with a new `OperatorCapabilityError` listing every denied
capability (and how to fix it). Capability names are validated against
the `Capability` enum so manifest typos surface as warnings instead of
silently passing.

*Per-operator rate limiting.* Manifests accept a `rate_limit_rpm` field
(0 = unlimited). The scheduler reads it from task metadata and, when a
tick fires sooner than the minimum interval, skips execution and pushes
`next_run` past the interval instead of spinning. The limit uses the
persisted last-run timestamp, so it holds across restarts.

*Failure circuit breaker.* A new `max_consecutive_failures` manifest
field auto-pauses an operator after that many consecutive failed runs
(disabled by default). The failure streak is derived from the persisted
run logs, so it survives restarts, and pausing publishes a new
`scheduler_task_paused` event on the bus. Resume with `nova operators
resume`.

*Health reporting.* `OperatorManager.health()` derives a per-operator
health label (`healthy` / `degraded` / `failing` / `stalled` /
`paused` / `pending` / `inactive`) from task state plus recent run
logs, with consecutive-failure counts and last-error text. Surfaced via
a new `nova operators status` table command, and guardrail fields now
appear in `nova operators info`.

**Vision input for `nova ask`** — attach images to a query with
...
`NOVA_NUM_CTX` environment variable to tune the Ollama context window
(default `16384`).

### Fixed

**`nova operators logs` crashed with `AttributeError`.** The command
called `SchedulerStore.get_runs()`, which exists only on a test fake;
the real store method is `get_run_logs()`. Every invocation fell into
the generic error handler. Now calls the real method, with CLI tests
covering both populated and empty histories.
...
install path while `nova_ai.ai` is down, improves desktop
first-boot diagnostics on Windows, and ships the RAM-detection fix
for Windows that missed the v1.0.1 cutoff.

### Fixed

**`nova_ai/traces/` missing from the v1.0.1 PyPI wheel** (#372).
...
`src/nova_ai/traces/` — silently dropping the whole package. Every
fresh `pip install nova_ai==1.0.1` then failed at import with
`ModuleNotFoundError: No module named 'nova_ai.traces'` on the
first `nova ask`, learning, or server call. Anchored the pattern to
...
the v1.0.1 cutoff, so v1.0.1 users still saw `0.0 GB` from `nova
...
`nova_ai.ai/install.sh` URL was failing with `sslv3 alert
...
`https://hamza35779.github.io/NOVA-AI/install.sh`, generated from
...
**`nova self-update`** subcommand. Detects how NOVA AI was
installed (pip, uv tool, editable git checkout) by inspecting
`nova_ai.__file__`, then runs the right upgrade command. Supports
...
(`NOVA_AI_NO_UPDATER=1`) documented in `docs/desktop-auto-update.md`.

**Analytics env-var opt-out** (`DO_NOT_TRACK`, `NOVA_AI_NO_ANALYTICS`).
...
upgrade command. Honors `NOVA_NO_UPDATE_CHECK=1` and `CI=true` to
...
- **Editable-git users running `nova self-update`** get the
  detected `git pull && uv sync` command pointed at their actual
  checkout, not `~/NOVA AI`. If you'd come to rely on the
...
`NovaSystem` in `src/nova_ai/system.py`, driven by a TOML config.
...
`nova init --preset <name>` bundle an agent with a hardware-appropriate
...
(`src/nova_ai/learning/routing/complexity.py`). Produces a 0.0–1.0
...
**LLM-guided spec search** (`src/nova_ai/learning/spec_search/`).
...
preset and a runnable tutorial at `examples/nova_ai/spec_search_quickstart.py`.

**Six hybrid coordination paradigms** in `src/nova_ai/agents/hybrid/`.
...
A runner CLI (`python -m nova_ai.agents.hybrid.runner --cell <name>`)
...
- **agentskills.io spec adoption** — canonical `SKILL.md` format with YAML frontmatter following the [agentskills.io](https://agentskills.io/specification) open standard.
  ...
  - Sourced subdirectory layout (`~/.nova_ai/skills/<source>/<name>/`)
...
- **Skills learning loop** — trace tagging, pattern discovery, DSPy/GEPA optimization.
  ...
  - `SkillOverlay` — sidecar storage at `~/.nova_ai/learning/skills/<name>/optimized.toml`
...
- **Skills benchmark harness** — 4-condition PinchBench evaluation.
  ...
  - `NovaAgentBackend` accepts `skills_enabled` and `overlay_dir` kwargs
  - Conditions: `no_skills`, `skills_on`, `skills_optimized_dspy`, `skills_optimized_gepa`

- **CLI commands:**
  - `nova skill list` / `info` / `run` / `install` / `sync` / `sources` / `update` / `remove` / `search`
  - `nova skill discover` — mine traces for recurring tool patterns
  - `nova skill show-overlay` — inspect optimization output
  - `nova optimize skills` — run DSPy/GEPA per-skill optimization
  - `nova bench skills` — run the PinchBench skills benchmark
...
- `examples/nova_ai/spec_search_quickstart.py` — runnable end-to-end
...
- `src/nova_ai/agents/hybrid/registry/*.toml` — 35-cell registry of
...
- **`learning/distillation/` is now `learning/spec_search/`.** The
  ...
  (`from nova_ai.learning.distillation.*` →
  `from nova_ai.learning.spec_search.*`). The `nova distillation`
...
- **Engine `generate_full` return shape extended.**
  `NovaAgentBackend.generate_full` and `NovaDirectBackend.generate_full`
...
- **OpenClaw category/name queries** — `nova skill install openclaw:owner/slug` now correctly splits into category + name match.
